...
test.describe('MYDS Accessibility Compliance Tests', () => {
  ...
  test.describe('Notification Components Accessibility', () => {
    test('notification bell component should be accessible', async ({
      page,
    }) => {
      // Look for notification bell button
      const notificationBell = page.locator(
        'button[aria-label*="notifikasi"], button[aria-label*="notifications"]'
      );
      ...
      if ((await notificationBell.count()) > 0) {
        ...
        const notificationBadge = notificationBell
          .locator('span')
          .filter({ hasText: /\d+/ });
        ...
        const dropdown = page.locator(
          '[role="menu"], div[aria-orientation="vertical"]'
        );
    ...
    test('notification center should have proper accessibility structure', async ({
      page,
    }) => {
      ...
      if ((await notificationCenterLink.count()) > 0) {
        ...
        if ((await notificationList.count()) > 0) {
          ...
          const notificationIcon = firstNotification.locator(
    ...
    test('system notification bar should be accessible', async ({ page }) => {
      ...
      const systemNotificationBar = page.locator(
        '[role="banner"], [role="alert"]'
      );
  ...
  test.describe('Form Components Accessibility', () => {
    test('form inputs should have proper labels and ARIA attributes', async ({
      page,
    }) => {
    ...
    test('form validation should be accessible', async ({ page }) => {
      ...
      if ((await validationForm.count()) > 0) {
        ...
        const requiredInputs = validationForm.locator(
        ...
        for (let i = 0; i < (await requiredInputs.count()); i++) {
          ...
          if (inputId) {
            const errorMessage = page.locator(
              `[id="${inputId}-error"], [aria-describedby*="${inputId}"]`
            );
      ...
      if ((await modalTrigger.count()) > 0) {
        ...
        if ((await modal.count()) > 0) {
          ...
          const modalTitle = modal
            .locator('h1, h2, h3, [aria-labelledby]')
            .first();
    ...
    test('dropdown menus should be keyboard navigable', async ({ page }) => {
      ...
      for (let i = 0; i < Math.min(await dropdownTriggers.count(), 3); i++) {
        ...
        if ((await dropdown.count()) > 0) {
          ...
          const menuItems = dropdown.locator(
  ...
  test.describe('Color Contrast and Visual Accessibility', () => {
    ...
    test('interactive elements should have minimum touch target size', async ({
      page,
    }) => {
