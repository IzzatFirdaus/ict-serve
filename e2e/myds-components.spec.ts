...
test.describe('MYDS Components Test Suite', () => {
  ...
  test('should display MYDS test page with proper headings', async ({ page }) => {
    // Check if the main heading is visible (use first() to avoid strict mode violation)
    await expect(page.locator('h1').first()).toContainText('MYDS Component Library Test');

    // Check if all section headings are present using more specific selectors
    await expect(page.locator('h2').filter({ hasText: 'Grid System' })).toBeVisible();
    ...
    await expect(page.locator('h2').filter({ hasText: 'Form Components' })).toBeVisible();
  });
  ...
  test('should verify button components and interactions', async ({ page }) => {
    ...
    const primaryBtn = page.locator('button').filter({ hasText: 'Primary Button' });
    const secondaryBtn = page.locator('button').filter({ hasText: 'Secondary Button' });
    const outlineBtn = page.locator('button').filter({ hasText: 'Outline Button' });
    const ghostBtn = page.locator('button').filter({ hasText: 'Ghost Button' });
    const dangerBtn = page.locator('button').filter({ hasText: 'Danger Button' });
  ...
  test('should verify alert components', async ({ page }) => {
    ...
    const infoAlert = page.locator('.bg-primary-50').filter({ hasText: 'Information' });
    const successAlert = page.locator('.bg-success-50').filter({ hasText: 'Success' });
    const warningAlert = page.locator('.bg-warning-50').filter({ hasText: 'Warning' });
    const dangerAlert = page.locator('.bg-danger-50').filter({ hasText: 'Error' });
  ...
  test('should verify badge components', async ({ page }) => {
    ...
    const primaryBadge = page.locator('.bg-primary-600, .bg-primary-100').filter({ hasText: 'Primary' });
    const successBadge = page.locator('.bg-success-600, .bg-success-100').filter({ hasText: 'Success' });
    const warningBadge = page.locator('.bg-warning-600, .bg-warning-100').filter({ hasText: 'Warning' });
    const dangerBadge = page.locator('.bg-danger-600, .bg-danger-100').filter({ hasText: 'Danger' });
  ...
  test('should verify tabs functionality', async ({ page }) => {
    ...
    // Test tab switching
    await tab2.click();
    const tab2Content = page.locator('text=This is the content of the second tab');
    await expect(tab2Content).toBeVisible();

    await tab3.click();
    const tab3Content = page.locator('text=This is the content of the third tab');
  ...
  test('should verify table component', async ({ page }) => {
    ...
    // Check table data
    await expect(page.locator('td').filter({ hasText: 'Ahmad Ibrahim' })).toBeVisible();
    await expect(page.locator('td').filter({ hasText: 'ahmad@motac.gov.my' })).toBeVisible();
  });
  ...
  test('should verify breadcrumb navigation', async ({ page }) => {
    ...
    await expect(page.locator('a').filter({ hasText: 'ServiceDesk ICT' })).toBeVisible();
    await expect(page.locator('a').filter({ hasText: 'Equipment Loans' })).toBeVisible();
  ...
  test('should verify dropdown functionality', async ({ page }) => {
    ...
    await page.locator('h2').filter({ hasText: 'Interactive Components' }).scrollIntoViewIfNeeded();
    ...
    await expect(page.locator('[role="menuitem"]').filter({ hasText: 'View Details' })).toBeVisible();
    await expect(page.locator('[role="menuitem"]').filter({ hasText: 'Edit' })).toBeVisible();
    await expect(page.locator('[role="menuitem"]').filter({ hasText: 'Download' })).toBeVisible();
  ...
  test('should verify tooltip functionality', async ({ page }) => {
    ...
    const tooltipBtn = page.locator('button').filter({ hasText: 'Hover for tooltip' });
  ...
  test('should verify modal functionality', async ({ page }) => {
    ...
    await expect(modal.locator('h3').filter({ hasText: 'Test Modal' })).toBeVisible();
  ...
  test('should verify MYDS color tokens usage', async ({ page }) => {
    ...
    const primaryElements = page.locator('.bg-primary-600, .text-txt-primary, .border-primary-600');
    expect(await primaryElements.count()).toBeGreaterThan(0);

    const successElements = page.locator('.bg-success-50, .text-txt-success, .border-success-200');
    expect(await successElements.count()).toBeGreaterThan(0);

    const warningElements = page.locator('.bg-warning-50, .text-txt-warning, .border-warning-200');
    expect(await warningElements.count()).toBeGreaterThan(0);

    const dangerElements = page.locator('.bg-danger-50, .text-txt-danger, .border-danger-200');
