...
test.describe('Notification Component Accessibility Tests', () => {
  ...
  test('notification bell should be accessible and functional', async ({
    page,
  }) => {
    // Look for notification bell component across different possible pages
    const notificationBell = page.locator(
      '[data-testid="notification-bell"], .notification-bell, .notification-dropdown'
    );
    ...
    if ((await notificationBell.count()) > 0) {
      ...
      const ariaLabel = await notificationBell
        .first()
        .getAttribute('aria-label');
      const ariaExpanded = await notificationBell
        .first()
      ...
      const dropdown = page.locator(
        '.dropdown-menu, [role="menu"], .notification-dropdown-menu'
      );
  ...
  test('notification system should have proper ARIA live regions', async ({
    page,
  }) => {
    ...
    if ((await liveRegions.count()) > 0) {
      ...
      for (let i = 0; i < (await liveRegions.count()) && i < 5; i++) {
        ...
        console.log(
          `✓ Live region ${i + 1}: aria-live="${ariaLive}", role="${role}"`
        );
      }
    } else {
      console.log(
        '⚠ No ARIA live regions found - notifications may not be announced to screen readers'
      );
    }
  });

  test('notification icons should have proper text alternatives', async ({
    page,
  }) => {
    ...
    for (
      let i = 0;
      i < Math.min(await notificationIcons.count(), maxIcons);
      i++
    ) {
      ...
      if (await icon.isVisible()) {
        ...
        if (!hasTextAlternative && !isHidden) {
          console.warn(
            `⚠ Icon ${i + 1} missing text alternative and not hidden from screen readers`
          );
        } else {
          iconCount++;
        }
  ...
  test('notification colors should have sufficient contrast', async ({
    page,
  }) => {
    ...
    if ((await notificationElements.count()) > 0) {
      for (
        let i = 0;
        i < Math.min(await notificationElements.count(), 5);
        i++
      ) {
        ...
        if (await element.isVisible()) {
          ...
          console.log(
            `✓ Element ${i + 1}: color="${styles.color}", background="${styles.backgroundColor}"`
          );
        }
      }
    } else {
    ...
    for (let i = 0; i < Math.min(await formInputs.count(), 10); i++) {
      ...
      if (await input.isVisible()) {
        ...
        if (hasLabel || ariaLabel || ariaLabelledBy) {
          ...
          console.warn(
            `⚠ Input missing label: type="${inputType}", name="${inputName}"`
          );
        }
      }
    }
...
test.describe('Basic MYDS Compliance Tests', () => {
  test('page should use MYDS semantic tokens', async ({ page }) => {
    ...
    if (mydsCount > 0) {
      ...
      console.log(
        '⚠ No MYDS semantic tokens found - page may not be MYDS compliant'
      );
    }
  });

  test('interactive elements should meet minimum size requirements', async ({
    page,
  }) => {
    await page.goto('/');
    await page.waitForLoadState('networkidle');
      ...
      if (await button.isVisible()) {
        ...
        if (boundingBox) {
          ...
          if (boundingBox.width < minSize || boundingBox.height < minSize) {
            ...
            console.warn(
              `⚠ Small interactive element: ${text?.slice(0, 30)}... (${Math.round(boundingBox.width)}x${Math.round(boundingBox.height)})`
            );
          }
        }
      }
