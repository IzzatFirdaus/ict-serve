...
test.describe('ICTServe Component Library', () => {
  ...
  test.describe('Dashboard Widgets', () => {
    test('statistics widget display', async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="stats-widget"]').first()
      ).toBeVisible();
    ...
    test('recent activities display', async ({ page }) => {
      ...
      const activitiesWidget = page.locator(
  ...
  test.describe('Form Components', () => {
    test('loan application form validation and submission', async ({
      page,
    }) => {
      await page.goto('/loans/create');
    ...
    test('helpdesk ticket form functionality', async ({ page }) => {
      ...
      await page.fill(
        'input[name="subject"]',
        'Masalah Komputer Tidak Boleh Boot'
      );
      await page.fill(
        'textarea[name="description"]',
        'Komputer tidak boleh boot selepas restart'
      );
      ...
      await page.fill(
        'textarea[name="attempted_solutions"]',
        'Sudah cuba restart beberapa kali'
      );
  ...
  test.describe('Data Table Component', () => {
    ...
    test('data table sorting functionality', async ({ page }) => {
      ...
      await sortableHeader.click();
      await expect(
        sortableHeader.locator('.sort-indicator.desc')
      ).toBeVisible();
    });
    ...
    test('row selection functionality', async ({ page }) => {
      ...
      const firstCheckbox = page
        .locator('.data-table tbody input[type="checkbox"]')
        .first();
  ...
  test.describe('Status Tracker Component', () => {
    test('status tracker display', async ({ page }) => {
      ...
      await expect(page.locator('.status-tracker .step.current')).toHaveCount(
        1
      );
  ...
  test.describe('Modal Components', () => {
    test('modal dialog functionality', async ({ page }) => {
      ...
      await expect(
        page.locator('.modal-content button:first-child')
      ).toBeFocused();
  ...
  test.describe('Search Component', () => {
    test('global search functionality', async ({ page }) => {
      ...
      await expect(
        page.locator('.search-component .search-results')
      ).toBeVisible();
      ...
      if ((await searchResults.count()) > 0) {
        await expect(
          searchResults.first().locator('.result-title')
        ).toBeVisible();
        await expect(
          searchResults.first().locator('.result-description')
        ).toBeVisible();
      }
  ...
  test.describe('File Upload Component', () => {
    test('file upload drag and drop', async ({ page }) => {
      ...
      await expect(
        page.locator('.file-upload-component .file-item')
      ).toBeVisible();
      await expect(
        page.locator('.file-upload-component .file-item')
