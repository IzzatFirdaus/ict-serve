...
test.describe('Loan Application Wizard', () => {
  ...
  test('should complete full 8-step loan application process', async () => {
    ...
    await expect(helpers.page.locator('[data-testid="loan-wizard"]')).toBeVisible();
    ...
    await helpers.testFormValidationDetailed([
      ...
      { field: 'ic_number', value: '123', expectedError: 'Nombor IC tidak sah' },
      { field: 'email', value: 'invalid-email', expectedError: 'Format email tidak sah' }
    ]);
    ...
    await helpers.page.fill('[name="purpose_description"]', 'Testing equipment for system upgrade project');
    ...
    const equipmentCard = helpers.page.locator('[data-testid="equipment-card"]').first();
    await equipmentCard.waitFor();
    await equipmentCard.click();

    // Verify equipment is added to selection
    await expect(helpers.page.locator('[data-testid="selected-equipment"]')).toContainText('laptop');
    ...
    await helpers.page.selectOption('[name="transport_method"]', 'self_collect');
    ...
    // Verify file upload
    await expect(helpers.page.locator('[data-testid="uploaded-files"]')).toContainText('test-document.pdf');
    ...
    await expect(helpers.page.locator('[data-testid="review-personal-info"]')).toContainText('John Doe Tester');
    await expect(helpers.page.locator('[data-testid="review-equipment"]')).toContainText('laptop');
    await expect(helpers.page.locator('[data-testid="review-dates"]')).toContainText('2024-12-15');

    // Submit application
    await helpers.page.click('[data-testid="submit-application"]');

    // Verify success message and redirect
    await expect(helpers.page.locator('[data-testid="success-message"]')).toBeVisible();
    await expect(helpers.page.locator('[data-testid="application-reference"]')).toBeVisible();
    ...
    for (let step = 1; step <= 3; step++) {
      ...
      if (step < 3) {
        ...
        if (step === 1) {
          ...
          await helpers.page.selectOption('[name="loan_purpose"]', 'official_duty');
          await helpers.page.fill('[name="purpose_description"]', 'Test purpose');
          await helpers.page.fill('[name="start_date"]', '2024-12-15');
          await helpers.page.fill('[name="end_date"]', '2024-12-20');
        }
  ...
  test('should validate required fields on each step', async () => {
    ...
    // Step 1 validation
    await helpers.page.click('[data-testid="next-step"]');
    await expect(helpers.page.locator('[data-testid="error-full_name"]')).toBeVisible();
    ...
    // Step 2 validation
    await helpers.page.click('[data-testid="next-step"]');
    await expect(helpers.page.locator('[data-testid="error-loan_purpose"]')).toBeVisible();
    await expect(helpers.page.locator('[data-testid="error-start_date"]')).toBeVisible();
  });
  ...
  test('should save progress and allow resuming', async () => {
    ...
    await expect(helpers.page.locator('[data-testid="draft-saved"]')).toBeVisible();
    ...
    await expect(helpers.page.locator('[name="full_name"]')).toHaveValue('Test User');
    await expect(helpers.page.locator('[name="ic_number"]')).toHaveValue('901234567890');
  });
