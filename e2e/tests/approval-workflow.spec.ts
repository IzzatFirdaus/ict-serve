...
test.describe('Approval Workflow', () => {
  ...
  test.describe('Staff User Workflow', () => {
    ...
    test('should view pending approval requests as staff', async () => {
      ...
      await expect(helpers.page.locator('[data-testid="pending-approvals"]')).toBeVisible();

      // Verify approval cards are displayed with proper information
      const approvalCards = helpers.page.locator('[data-testid="approval-card"]');
      if (await approvalCards.count() > 0) {
        const firstCard = approvalCards.first();
        await expect(firstCard.locator('[data-testid="application-id"]')).toBeVisible();
        await expect(firstCard.locator('[data-testid="applicant-name"]')).toBeVisible();
        await expect(firstCard.locator('[data-testid="equipment-requested"]')).toBeVisible();
        await expect(firstCard.locator('[data-testid="request-date"]')).toBeVisible();
        await expect(firstCard.locator('[data-testid="approval-status"]')).toBeVisible();
      }

      // Test filtering by status
      const statusFilters = ['pending', 'approved', 'rejected', 'returned'];
      for (const status of statusFilters) {
        const filterButton = helpers.page.locator(`[data-testid="filter-${status}"]`);
        if (await filterButton.isVisible()) {
          await filterButton.click();
          await helpers.page.waitForTimeout(500);

          // Verify filtered results
          const filteredCards = helpers.page.locator('[data-testid="approval-card"]:visible');
          if (await filteredCards.count() > 0) {
            const statusIndicator = await filteredCards.first().locator('[data-testid="approval-status"]').textContent();
    ...
    test('should view detailed approval information', async () => {
      ...
      const firstCard = helpers.page.locator('[data-testid="approval-card"]').first();
      if (await firstCard.isVisible()) {
        await firstCard.click();

        // Verify approval details modal/page opens
        await expect(helpers.page.locator('[data-testid="approval-details"]')).toBeVisible();

        // Check all required information is displayed
        await expect(helpers.page.locator('[data-testid="applicant-details"]')).toBeVisible();
        await expect(helpers.page.locator('[data-testid="equipment-list"]')).toBeVisible();
        await expect(helpers.page.locator('[data-testid="loan-purpose"]')).toBeVisible();
        await expect(helpers.page.locator('[data-testid="loan-dates"]')).toBeVisible();
        await expect(helpers.page.locator('[data-testid="supporting-documents"]')).toBeVisible();

        // Test document viewing
        const documentLinks = helpers.page.locator('[data-testid="document-link"]');
  ...
  test.describe('Supervisor Approval Workflow', () => {
    ...
    test('should approve loan application', async () => {
      ...
      const pendingCard = helpers.page.locator('[data-testid="approval-card"]')
        .filter({ has: helpers.page.locator('[data-testid="approval-status"]:has-text("Pending")') })
        .first();
      ...
      if (await pendingCard.isVisible()) {
        ...
        await helpers.page.fill('[data-testid="approval-comments"]', 'Application reviewed and approved. Equipment is available for the requested dates.');

        // Set loan conditions if needed
        await helpers.page.fill('[data-testid="loan-conditions"]', 'Equipment must be returned in original condition. No modifications allowed.');

        // Approve the application
        await helpers.page.click('[data-testid="approve-button"]');

        // Verify approval confirmation
        await expect(helpers.page.locator('[data-testid="approval-success"]')).toBeVisible();

        // Check status update
        await expect(helpers.page.locator('[data-testid="approval-status"]')).toContainText('Approved');
      }
    });
    ...
    test('should reject loan application with reasons', async () => {
      ...
      if (await pendingCard.isVisible()) {
        ...
        await helpers.page.fill('[data-testid="rejection-reason"]', 'Equipment not available for requested dates. Please select alternative dates.');

        // Select rejection category
        await helpers.page.selectOption('[data-testid="rejection-category"]', 'unavailable_equipment');
        ...
        await expect(helpers.page.locator('[data-testid="rejection-success"]')).toBeVisible();

        // Check status update
        await expect(helpers.page.locator('[data-testid="approval-status"]')).toContainText('Rejected');
      }
    });
    ...
    test('should request additional information', async () => {
      ...
      // Find a pending approval
      const pendingCard = helpers.page.locator('[data-testid="approval-card"]').first();
      ...
      if (await pendingCard.isVisible()) {
        ...
        await helpers.page.fill('[data-testid="info-request-message"]', 'Please provide additional justification for the loan request and updated project timeline.');
        ...
        // Verify request sent
        await expect(helpers.page.locator('[data-testid="info-request-sent"]')).toBeVisible();

        // Check status update
        await expect(helpers.page.locator('[data-testid="approval-status"]')).toContainText('Information Requested');
      }
    });
  });
  ...
  test.describe('Admin Approval Management', () => {
    ...
    test('should manage approval workflow settings', async () => {
      ...
      await expect(helpers.page.locator('[data-testid="approval-settings"]')).toBeVisible();
      ...
      await helpers.page.selectOption('[data-testid="level-1-approver"]', 'supervisor');
      await helpers.page.selectOption('[data-testid="level-2-approver"]', 'manager');
    ...
    test('should view approval analytics and reports', async () => {
      ...
      await expect(helpers.page.locator('[data-testid="approval-analytics"]')).toBeVisible();

      // Verify key metrics are displayed
      await expect(helpers.page.locator('[data-testid="total-applications"]')).toBeVisible();
      await expect(helpers.page.locator('[data-testid="pending-approvals"]')).toBeVisible();
      await expect(helpers.page.locator('[data-testid="approval-rate"]')).toBeVisible();
      await expect(helpers.page.locator('[data-testid="average-processing-time"]')).toBeVisible();
      ...
      await expect(helpers.page.locator('[data-testid="approval-chart"]')).toBeVisible();
    ...
    test('should manage approval delegates and permissions', async () => {
      ...
      await expect(helpers.page.locator('[data-testid="delegate-management"]')).toBeVisible();

      // Add new delegate
      await helpers.page.click('[data-testid="add-delegate"]');
      await helpers.page.selectOption('[data-testid="delegate-supervisor"]', 'supervisor@example.com');
      await helpers.page.selectOption('[data-testid="delegate-to"]', 'assistant@example.com');
      await helpers.page.fill('[data-testid="delegate-start-date"]', '2024-12-01');
      await helpers.page.fill('[data-testid="delegate-end-date"]', '2024-12-31');
      ...
      await expect(helpers.page.locator('[data-testid="delegate-saved"]')).toBeVisible();
    });
  });
  ...
  test.describe('Workflow Integration Tests', () => {
    ...
    test('should test complete approval workflow cycle', async () => {
      ...
      await helpers.page.fill('[name="purpose_description"]', 'Testing workflow integration');
      ...
      const appRef = await helpers.page.locator('[data-testid="application-reference"]').textContent();
      ...
      const targetApp = helpers.page.locator('[data-testid="approval-card"]')
        .filter({ has: helpers.page.locator(`[data-testid="application-id"]:has-text("${appRef}")`) });

      if (await targetApp.isVisible()) {
        await targetApp.click();
        await helpers.page.fill('[data-testid="approval-comments"]', 'Approved for testing workflow');
        await helpers.page.click('[data-testid="approve-button"]');
      ...
      const myApp = helpers.page.locator('[data-testid="my-application"]')
        .filter({ has: helpers.page.locator(`[data-testid="app-reference"]:has-text("${appRef}")`) });

      if (await myApp.isVisible()) {
        await expect(myApp.locator('[data-testid="app-status"]')).toContainText('Approved');
      }
    });
    ...
    test('should test escalation workflow', async () => {
      ...
      const managerApproval = helpers.page.locator('[data-testid="approval-card"]')
        .filter({ has: helpers.page.locator('[data-testid="escalation-indicator"]') });

      if (await managerApproval.isVisible()) {
        await expect(managerApproval.locator('[data-testid="approval-level"]')).toContainText('Manager');
      }
    });
    ...
    test('should test bulk approval operations', async () => {
      ...
      const pendingCards = helpers.page.locator('[data-testid="approval-card"]')
        .filter({ has: helpers.page.locator('[data-testid="approval-status"]:has-text("Pending")') });

      const cardCount = Math.min(3, await pendingCards.count());

      for (let i = 0; i < cardCount; i++) {
        await pendingCards.nth(i).locator('[data-testid="select-application"]').check();
      }

      // Test bulk approval
      await helpers.page.click('[data-testid="bulk-approve"]');
      await helpers.page.fill('[data-testid="bulk-approval-comment"]', 'Bulk approved for routine requests');
