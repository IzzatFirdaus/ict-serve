...
test.describe('Static Page Accessibility Tests', () => {
  ...
  test('login page should use MYDS semantic tokens', async ({ page }) => {
    ...
    const mydsElements = page.locator('[class*="bg-primary"], [class*="text-primary"], [class*="border-primary"], [class*="myds-"]');
    const count = await mydsElements.count();
    expect(count).toBeGreaterThan(0);
    
    console.log(`✓ Found ${count} elements using MYDS semantic tokens or components`);
  });

  test('login page interactive elements should meet minimum size requirements', async ({ page }) => {
    await page.goto('/login');
      ...
      if (isVisible) {
        ...
        if (boundingBox) {
          ...
          if (width < 24 || height < 24) {
            ...
            console.log(`⚠ Small interactive element: ${text.trim().substring(0, 30)}... (${Math.round(width)}x${Math.round(height)})`);
          }
        }
      }
    }
  ...
  test('homepage should be accessible without authentication', async ({ page }) => {
    ...
    try {
      ...
      if (response && response.status() === 200) {
        ...
        if (count > 0) {
          console.log(`✓ Found ${count} elements using MYDS semantic tokens on homepage`);
        }
      }
    } catch (error) {
