...
document.addEventListener('alpine:init', () => {
    ...
    Alpine.data('formValidation', () => ({
        ...
        isTouched(field) { return this.touched.hasOwnProperty(field); },
    ...
    Alpine.data('progress', (initialValue = 0, max = 100) => ({
        ...
        max: max,
        percentage: 0,
        init() { this.updatePercentage(); },
        setValue(newValue) { this.value = Math.min(Math.max(0, newValue), this.max); this.updatePercentage(); },
        updatePercentage() { this.percentage = (this.value / this.max) * 100; },
    ...
    Alpine.data('characterCounter', (maxLength) => ({
        count: 0,
        maxLength: maxLength,
        updateCount(value) { this.count = value ? value.length : 0; },
        isOverLimit() { return this.count > this.maxLength; },
        remainingCharacters() { return this.maxLength - this.count; }
    }));
    ...
    Alpine.data('fileUpload', (maxFiles = 5, allowedTypes = []) => ({
        ...
        clearFiles() { this.files = []; this.errors = []; },
...
window.MYDS = {
    ...
    // Escape key handler
    onEscape(callback) {
        document.addEventListener('keydown', (e) => { if (e.key === 'Escape') { callback(); } });
    },
    ...
    getTheme() { return localStorage.getItem('myds-theme') || 'light'; },
    initTheme() { const savedTheme = this.getTheme(); this.setTheme(savedTheme); }
};
// Inisialisasi tema pada muat halaman
document.addEventListener('DOMContentLoaded', () => { window.MYDS.initTheme(); });
