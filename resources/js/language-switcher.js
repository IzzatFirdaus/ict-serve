...
class LanguageSwitcher {
    ...
    setupButtonLanguageSwitcher(button) {
        ...
        button.setAttribute('aria-label', 
            `Switch to ${this.localeLabels[locale]} / Tukar kepada ${this.localeLabels[locale]}`);
        button.setAttribute('aria-pressed', (locale === this.currentLocale).toString());
    ...
    switchLanguage(locale) {
        ...
        this.dispatchLanguageEvent('beforeSwitch', { from: this.currentLocale, to: locale });
    ...
    buildLanguageRoute(locale) {
        ...
        if (typeof route !== 'undefined') {
            try {
                ...
                console.warn('Laravel route helper not available for language switching');
            }
        }
    ...
    updateLanguageIndicators() {
        ...
        const linkElements = document.querySelectorAll('[data-language-link], [data-language-button]');
        linkElements.forEach(element => {
            const locale = element.dataset.languageLink || element.dataset.languageButton;
    ...
    dispatchLanguageEvent(eventType, detail) {
        const event = new CustomEvent(`language${eventType.charAt(0).toUpperCase() + eventType.slice(1)}`, {
