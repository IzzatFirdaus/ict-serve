...
document.addEventListener('livewire:init', function () {
  const isLivewireAvailable =
    window.Livewire && typeof window.Livewire.on === 'function';
  ...
  if (isLivewireAvailable) {
    window.Livewire.on('system-notification', function (notification) {
      ...
      try {
        const region =
          document.querySelector('[role="status"]') || document.body;
        const sr = document.createElement('div');
        sr.className = 'sr-only';
        sr.setAttribute('aria-live', 'polite');
        sr.textContent = notification?.title
  ...
  document.addEventListener('system-notification:show', function (e) {
    ...
    const timeout = parseInt(
      detail.timeout || detail.autoHideTimeout || 5000,
      10
    );
    ...
    document.dispatchEvent(
      new CustomEvent('auto-hide-notification', { detail: { timeout } })
    );
