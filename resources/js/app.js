...
document.addEventListener('DOMContentLoaded', function () {
  console.log(
    'ICT Serve (iServe) application initialized - MYDS/MyGOVEA compliant'
  );
  ...
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
      ...
      const openDropdowns = document.querySelectorAll(
        '[x-show="true"], .dropdown-open'
      );
      openDropdowns.forEach((dropdown) => {
        if (dropdown.hasAttribute('x-show')) {
          // Alpine.js dropdown
          dropdown.__x_dataStack?.[0]?.open &&
            (dropdown.__x_dataStack[0].open = false);
        } else {
...
function initializeAnalytics() {
  ...
  console.log(
    'Analytics initialization placeholder - MYDS/MyGOVEA compliance required'
  );
}
...
window.showNotification = function (message, type = 'info') {
  console.warn(
    'showNotification is deprecated. Use toastManager methods instead.'
  );
