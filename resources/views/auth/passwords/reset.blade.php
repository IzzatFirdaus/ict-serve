...
<div id="main-content" class="myds-container max-w-md mx-auto py-12" tabindex="-1">
    <div class="bg-bg-white-0 border border-otl-gray-200 rounded-lg shadow-context-menu p-8">
        <h1 class="myds-heading-md font-semibold text-txt-black-900 mb-1">{{ __('Pilih Kata Laluan Baharu') }}</h1>
        <p class="myds-body-sm text-txt-black-600 mb-6">{{ __('Masukkan emel dan kata laluan baharu anda') }}</p>

        @if (Route::has('password.update'))
        <form method="POST" action="{{ route('password.update') }}" class="space-y-5" novalidate aria-describedby="reset-desc">
            @csrf
            ...
            <x-myds.input
                ...
                autocomplete="new-password"
            />
            <x-myds.button type="submit" variant="primary" class="w-full">{{ __('Tetapkan semula kata laluan') }}</x-myds.button>
        </form>
        @else
