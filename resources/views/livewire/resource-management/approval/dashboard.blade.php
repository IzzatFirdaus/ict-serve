<div class="bg-background-light dark:bg-background-dark min-h-screen">
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
            <div class="bg-white dark:bg-dialog rounded-lg p-4 border border-divider">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="font-inter text-xs text-black-500 dark:text-black-400">Menunggu Kelulusan</p>
                        <p class="font-poppins text-2xl font-semibold text-warning-600">{{ $statusCounts['pending_support'] }}</p>
                    </div>
            ...
            <div class="bg-white dark:bg-dialog rounded-lg p-4 border border-divider">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="font-inter text-xs text-black-500 dark:text-black-400">Diluluskan</p>
                        <p class="font-poppins text-2xl font-semibold text-success-600">{{ $statusCounts['approved'] }}</p>
                    </div>
            ...
            <div class="bg-white dark:bg-dialog rounded-lg p-4 border border-divider">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="font-inter text-xs text-black-500 dark:text-black-400">Ditolak</p>
                        <p class="font-poppins text-2xl font-semibold text-danger-600">{{ $statusCounts['rejected'] }}</p>
                    </div>
            ...
            <div class="bg-white dark:bg-dialog rounded-lg p-4 border border-divider">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="font-inter text-xs text-black-500 dark:text-black-400">Dikeluarkan</p>
                        <p class="font-poppins text-2xl font-semibold text-primary-600">{{ $statusCounts['issued'] }}</p>
                    </div>
                    <x-myds.icon name="arrow-outgoing" size="24" class="text-primary-500" />
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-dialog rounded-lg p-4 border border-divider">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="font-inter text-xs text-black-500 dark:text-black-400">Selesai</p>
                        <p class="font-poppins text-2xl font-semibold text-black-600 dark:text-black-400">{{ $statusCounts['completed'] }}</p>
                    </div>
        ...
        <!-- Filters and Search -->
        <div class="bg-white dark:bg-dialog rounded-lg shadow-sm border border-divider mb-6">
            ...
            <div class="px-6 py-4 bg-washed dark:bg-black-100">
                <div class="flex flex-col md:flex-row gap-4">
                    <!-- Status Filter -->
                    <div class="flex-1">
                        <label for="status_filter" class="font-inter text-xs font-medium text-black-700 dark:text-black-300 block mb-2">
                            Status
                        </label>
                        ...
                        <thead class="bg-washed dark:bg-black-100">
                            <tr>
                                ...
                                @if($status_filter === 'pending_support')
                                    <th class="px-6 py-3 text-right font-inter text-xs font-medium text-black-700 dark:text-black-300 uppercase tracking-wider">
                                        Tindakan
                                    </th>
                                @endif
                            </tr>
                        ...
                        <tbody class="divide-y divide-divider">
                            @foreach($loans as $loan)
                                <tr class="hover:bg-washed dark:hover:bg-black-100">
                                    <td class="px-6 py-4">
                                        <div>
                                            <p class="font-inter text-sm font-medium text-black-900 dark:text-white">
                                                {{ $loan->request_number }}
                                            </p>
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="space-y-1">
                                            @foreach($loan->loanItems as $item)
                                                <p class="font-inter text-xs text-black-600 dark:text-black-400">
                                    ...
                                    <td class="px-6 py-4">
                                        <div>
                                            <p class="font-inter text-sm text-black-900 dark:text-white">
                                    ...
                                    <td class="px-6 py-4">
                                        ...
                                        @endphp
                                        <span class="inline-flex px-2 py-1 text-xs font-medium rounded-full {{ $config[0] }} {{ $config[1] }}">
                                            {{ $config[2] }}
                                        </span>
                                    </td>
            ...
            @else
                <div class="px-6 py-12 text-center">
                    ...
                    <p class="font-inter text-sm text-black-500 dark:text-black-400">
                        Tiada permohonan {{ $status_filter === 'pending_support' ? 'menunggu kelulusan' : '' }} ditemui.
                    </p>
                </div>
            @endif
        </div>
    ...
    <div
        x-data="{ open: false }"
        x-on:open-reject-modal.window="open = true; $wire.set('rejectLoanId', $event.detail.loan_id)"
        x-show="open"
        x-cloak
