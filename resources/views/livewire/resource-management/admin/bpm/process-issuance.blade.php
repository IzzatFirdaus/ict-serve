                                    ...
                                    <div class="flex justify-between items-start">
                                        <div class="flex-1">
                                            <div class="flex items-center space-x-3 mb-2">
                                                ...
                                                <span class="inline-flex px-2 py-1 text-xs font-medium rounded-full bg-success-100 text-success-700">
                                                    Diluluskan
                                                </span>
                                            </div>
                                        ...
                                        >
                                            <x-myds.icon name="arrow-forward" size="14" class="mr-1" />
                                            Proses
                                        </x-myds.button>
                                    </div>
                                </div>
                        ...
                        <div class="px-6 py-4 space-y-4">
                            ...
                            <div class="bg-washed dark:bg-black-100 rounded-lg p-4">
                                ...
                                <div class="space-y-1 text-xs">
                                    ...
                                    <p><strong>Pemohon:</strong> {{ $selectedLoan->user->name }}</p>
                                    <p><strong>Tujuan:</strong> {{ $selectedLoan->purpose }}</p>
                                    <p><strong>Lokasi:</strong> {{ $selectedLoan->location ?? '-' }}</p>
                                </div>
                            </div>
                                    ...
                                    @foreach($selectedLoan->loanItems as $item)
                                        @if($item->equipmentItem)
                                            <div class="flex items-center space-x-3">
                                                ...
                                                />
                                                <label for="equipment_{{ $item->equipmentItem->id }}" class="font-inter text-xs text-black-700 dark:text-black-300 cursor-pointer">
                                                    ...
                                                    <span class="block text-black-500 dark:text-black-400">
                                                        ...
                                                        @if($item->equipmentItem->status !== 'available')
                                                            <span class="text-danger-600">({{ ucfirst($item->equipmentItem->status) }})</span>
                                                        @endif
                                                    </span>
                                                </label>
                                            </div>
                                ...
                                <div class="space-y-2">
                                    ...
                                    @foreach($commonAccessories as $key => $label)
                                        <div class="flex items-center space-x-3">
                                            ...
                                            />
                                            <label for="accessory_{{ $key }}" class="font-inter text-xs text-black-700 dark:text-black-300 cursor-pointer">
                                                {{ $label }}
                                            </label>
                                        </div>
                                    @endforeach
                            ...
                            <!-- Issue Notes -->
                            <div>
                                <label for="issue_notes" class="font-inter text-xs font-medium text-black-700 dark:text-black-300 block mb-2">
                                    Catatan Pengeluaran
                                </label>
                                <textarea
                                    id="issue_notes"
                            ...
                            <x-myds.button
                                ...
                                class="w-full"
                            >
                                <x-myds.icon name="arrow-outgoing" size="16" class="mr-2" wire:loading.remove />
