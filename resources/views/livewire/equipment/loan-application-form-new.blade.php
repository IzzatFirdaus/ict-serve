...
<x-layouts.app :title="$title">
  <div class="myds-container max-w-6xl mx-auto py-8">
    ...
    <header class="mb-8">
      ...
      >
        <div>
          ...
          <p
            class="text-body-sm text-txt-black-500 dark:text-txt-black-500 mt-1"
          >
      ...
      <x-myds.callout variant="info">
        <h3 class="font-medium text-txt-black-900 dark:text-txt-black-900 mb-2">
          Syarat-syarat Peminjaman
        </h3>
        <ul
          class="text-body-sm space-y-1 text-txt-black-700 dark:text-txt-black-700"
        >
          <li>
            • Borang mestilah diisi lengkap, terutamanya medan yang bertanda
            <span class="text-danger-600 dark:text-danger-400 font-medium">
              *
            </span>
          </li>
          <li>
            • Permohonan tertakluk kepada ketersediaan peralatan atas dasar
            'Siapa Cepat, Dia Dapat'
          </li>
          <li>
            • Permohonan akan diproses dalam tempoh tiga (3) hari bekerja
            selepas menerima borang lengkap
          </li>
          <li>
            • Pemohon bertanggungjawab sepenuhnya ke atas sebarang kehilangan
            atau kerosakan
          </li>
        </ul>
      </x-myds.callout>
    ...
    >
      ...
      <div class="relative mb-8">
        ...
        >
          <span>Pemohon</span>
          <span>Pegawai</span>
          <span>{{ __('literals.equipment') }}</span>
          <span>Sokongan</span>
          <span>Semak</span>
        </div>
      </div>
      ...
      <x-myds.panel variant="default" x-show="currentStep === 2">
        ...
        <div
          class="mb-6 p-4 bg-primary-25 dark:bg-primary-25 border border-primary-200 dark:border-primary-200 rounded-radius-m"
        >
          ...
          <p
            class="text-body-xs text-txt-black-600 dark:text-txt-black-600 mt-2"
          >
            Tandakan jika anda akan bertanggungjawab sendiri terhadap peralatan
            yang dipinjam.
          </p>
        </div>
      ...
      <x-myds.panel variant="default" x-show="currentStep === 3">
        ...
        <div class="space-y-6">
          ...
          <div class="overflow-x-auto">
            <table
              class="min-w-full border border-otl-gray-200 dark:border-otl-gray-200 rounded-radius-m"
            >
              <thead>
                <tr class="bg-bg-washed dark:bg-bg-washed">
                  <th
                    class="px-4 py-3 text-left text-body-xs font-medium text-txt-black-900 dark:text-txt-black-900"
                  >
                    Bil.
                  </th>
                  <th
                  ...
                  <th
                    class="px-4 py-3 text-center text-body-xs font-medium text-txt-black-900 dark:text-txt-black-900"
                  >
                    Tindakan
                  </th>
                </tr>
              ...
              <tbody
                class="divide-y divide-otl-gray-200 dark:divide-otl-gray-200"
              >
                @foreach ($equipmentRequests as $index => $request)
                  ...
                  >
                    <td
                      class="px-4 py-3 text-body-sm text-txt-black-900 dark:text-txt-black-900"
                    >
                      {{ $index + 1 }}
                    </td>
                    <td class="px-4 py-3">
                      <select
                        wire:model.live="equipmentRequests.{{ $index }}.equipment_type"
                      ...
                      @error('equipmentRequests.' . $index . '.equipment_type')
                        <div class="myds-field-error" role="alert">
                          {{ $message }}
                        </div>
                      @enderror
                    </td>
          ...
          @if (count($equipmentRequests) >= 10)
            <x-myds.callout variant="warning">
              <p class="text-body-sm">
                Maksimum 10 jenis peralatan sahaja boleh dipohon dalam satu
                permohonan. Jika memerlukan lebih banyak peralatan, sila buat
                permohonan berasingan.
              </p>
            </x-myds.callout>
          @endif
        </div>
      ...
      <x-myds.panel variant="warning" x-show="currentStep === 4">
        ...
        <div class="space-y-6">
          <x-myds.callout variant="info">
            <h3
              class="font-medium text-txt-black-900 dark:text-txt-black-900 mb-2"
            >
              Keperluan Sokongan
            </h3>
            <p class="text-body-sm text-txt-black-700 dark:text-txt-black-700">
              Permohonan mesti disokong oleh pegawai yang bergred
              sekurang-kurangnya Gred 41. Sokongan ini menunjukkan bahawa
              permohonan telah diluluskan oleh pihak atasan dan adalah untuk
              kegunaan rasmi.
            </p>
          </x-myds.callout>

          <!-- Endorsing Officer Information -->
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <x-myds.input
          ...
          <!-- Declaration -->
          <div
            class="bg-warning-25 dark:bg-warning-25 border border-warning-200 dark:border-warning-200 rounded-radius-m p-6"
          >
            <h3
              class="font-medium text-txt-black-900 dark:text-txt-black-900 mb-3"
            >
              Perakuan Pegawai Penyokong
            </h3>
            <div
              class="prose prose-sm text-txt-black-700 dark:text-txt-black-700 max-w-none text-body-sm"
            >
              <p>
                Saya dengan ini mengesahkan bahawa permohonan peminjaman
                peralatan ICT ini adalah untuk kegunaan rasmi dan saya menyokong
                permohonan ini.
              </p>
            </div>
            ...
            <div class="flex items-start space-x-3 mt-4">
              ...
              />
              <label
                for="endorsement_confirmation"
                class="text-body-sm text-txt-black-900 dark:text-txt-black-900 font-medium"
              >
                Saya mengesahkan sokongan ini sebagai pegawai yang
                bertanggungjawab. *
              </label>
            </div>
      ...
      <x-myds.panel variant="success" x-show="currentStep === 5">
        ...
        <div class="space-y-8">
            ...
            <div class="space-y-4">
              ...
              <div class="space-y-2 text-body-sm">
                ...
                <div>
                  ...
                  {{ $loan_start_date ? \Carbon\Carbon::parse($loan_start_date)->format('d/m/Y') : '-' }}
                </div>
                <div>
                  <strong>Tarikh Pulang:</strong>
                  {{ $expected_return_date ? \Carbon\Carbon::parse($expected_return_date)->format('d/m/Y') : '-' }}
                </div>
              </div>
            </div>

            <!-- Responsible Officer Summary -->
            <div class="space-y-4">
              <h3
                class="font-heading text-heading-xs font-medium text-txt-black-900 dark:text-txt-black-900 border-b border-otl-gray-200 dark:border-otl-gray-200 pb-2"
              >
              ...
              <div class="space-y-2 text-body-sm">
                @if ($sameAsApplicant)
                  <div
                    class="flex items-center text-primary-600 dark:text-primary-600"
                  >
                    <svg
                      class="w-4 h-4 mr-2"
                      fill="none"
          ...
          <div>
            <h3
              class="font-heading text-heading-xs font-medium text-txt-black-900 dark:text-txt-black-900 border-b border-otl-gray-200 dark:border-otl-gray-200 pb-2 mb-4"
            >
            ...
            <div class="overflow-x-auto">
              ...
              >
                <thead class="bg-bg-washed dark:bg-bg-washed">
                  <tr>
                    <th
                      class="px-4 py-2 text-left text-body-xs font-medium text-txt-black-900 dark:text-txt-black-900"
                    >
                      Bil.
                    </th>
                    <th
                  ...
                  @foreach ($equipmentRequests as $index => $request)
                    @if ($request['equipment_type'] && $request['quantity'])
                      <tr class="text-body-sm">
                        ...
                        <td class="px-4 py-2">
                          {{ $equipmentTypes[$request['equipment_type']] ?? $request['equipment_type'] }}
                        </td>
          ...
          <div>
            ...
            <div
              class="bg-{{ $endorsement_status === 'supported' ? 'success' : 'danger' }}-25 dark:bg-{{ $endorsement_status === 'supported' ? 'success' : 'danger' }}-25 p-4 rounded-radius-m"
            >
              <div class="flex items-center space-x-3 mb-2">
                <x-myds.tag
                  :variant="$endorsement_status === 'supported' ? 'success' : 'danger'"
                >
                  {{ $endorsement_status === 'supported' ? 'DISOKONG' : 'TIDAK DISOKONG' }}
                </x-myds.tag>
                <span class="text-body-sm font-medium">
                  oleh {{ $endorsing_officer_name }}
                </span>
              </div>
              <div
                class="text-body-xs text-txt-black-600 dark:text-txt-black-600"
              >
    ...
    @if (session()->has('success'))
      ...
      >
        <div
          class="bg-bg-white-0 dark:bg-bg-white-0 rounded-radius-l shadow-context-menu p-8 max-w-lg mx-4 text-center"
        >
          <div
          ...
          <div
            class="space-y-2 text-body-xs text-txt-black-500 dark:text-txt-black-500 mb-6"
          >
            <p>
              Rujukan:
              <strong class="text-txt-black-900 dark:text-txt-black-900">
                {{ session('reference_code') }}
              </strong>
            </p>
            <p>Permohonan akan diproses dalam tempoh 3 hari bekerja.</p>
            <p>
              Sila bawa borang yang lengkap dan ditandatangani semasa mengambil
              peralatan.
            </p>
          </div>
          <div class="space-x-3">
