<div class="container mx-auto px-4 py-6">
    ...
    <div class="mb-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-semibold font-poppins text-black-900">Pengurusan Tiket Helpdesk</h1>
                <p class="text-sm text-black-500 font-inter mt-1">Urus dan pantau semua tiket sokongan ICT</p>
            </div>
            <div class="flex items-center space-x-3">
                <x-myds.button wire:click="clearFilters" variant="secondary" size="medium">
        ...
        <div class="bg-white border border-divider rounded-lg p-4">
            <div class="flex items-center">
                ...
                <div class="ml-3">
                    ...
                    <p class="text-2xl font-semibold text-black-900">{{ number_format($counts['total']) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white border border-divider rounded-lg p-4">
            <div class="flex items-center">
                ...
                <div class="ml-3">
                    ...
                    <p class="text-2xl font-semibold text-black-900">{{ number_format($counts['open']) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white border border-divider rounded-lg p-4">
            <div class="flex items-center">
                ...
                <div class="ml-3">
                    ...
                    <p class="text-2xl font-semibold text-black-900">{{ number_format($counts['assigned']) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white border border-divider rounded-lg p-4">
            <div class="flex items-center">
                ...
                <div class="ml-3">
                    ...
                    <p class="text-2xl font-semibold text-black-900">{{ number_format($counts['overdue']) }}</p>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-divider">
                <thead class="bg-washed">
                    <tr>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-black-500 uppercase tracking-wider cursor-pointer" wire:click="sortBy('ticket_number')">
                            No. Tiket
                            @if($sortField === 'ticket_number')
                                <x-myds.icon name="chevron-{{ $sortDirection === 'asc' ? 'up' : 'down' }}" size="12" class="inline ml-1" />
                            @endif
                        </th>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-black-500 uppercase tracking-wider cursor-pointer" wire:click="sortBy('title')">
                            Tajuk
                            @if($sortField === 'title')
                                <x-myds.icon name="chevron-{{ $sortDirection === 'asc' ? 'up' : 'down' }}" size="12" class="inline ml-1" />
                            @endif
                        ...
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-black-500 uppercase tracking-wider cursor-pointer" wire:click="sortBy('priority')">
                            Keutamaan
                        ...
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-black-500 uppercase tracking-wider cursor-pointer" wire:click="sortBy('created_at')">
                            Tarikh Dicipta
                            @if($sortField === 'created_at')
                                <x-myds.icon name="chevron-{{ $sortDirection === 'asc' ? 'up' : 'down' }}" size="12" class="inline ml-1" />
                            @endif
                        </th>
                        <th scope="col" class="px-6 py-3 text-center text-xs font-medium text-black-500 uppercase tracking-wider">
                            Tindakan
                        </th>
                    </tr>
                </thead>
                    ...
                    @forelse($tickets as $ticket)
                        <tr class="hover:bg-washed transition-colors duration-200">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-primary-600">
                                <a href="{{ route('helpdesk.tickets.show', $ticket) }}" class="hover:underline">
                                    {{ $ticket->ticket_number }}
                                </a>
                            </td>
                            <td class="px-6 py-4 text-sm text-black-900">
                                <div class="max-w-xs truncate">{{ $ticket->title }}</div>
                                <div class="text-xs text-black-500">{{ $ticket->category->name ?? 'Tiada Kategori' }}</div>
                            </td>
                            <td class="px-6 py-4 text-sm text-black-900">
                                <div>{{ $ticket->user->name }}</div>
                                <div class="text-xs text-black-500">{{ $ticket->user->email }}</div>
                            </td>
                            ...
                            <td class="px-6 py-4 text-sm text-black-900">
                                @if($ticket->assignedTo)
                                    ...
                                    <div class="text-xs text-black-500">{{ $ticket->assigned_at?->format('d/m/Y H:i') }}</div>
                                @else
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-6 py-12 text-center">
                                <x-myds.icon name="document" size="48" class="mx-auto text-black-300 mb-4" />
                                <h3 class="text-lg font-medium text-black-900 mb-2">Tiada tiket ditemui</h3>
                                <p class="text-black-500">Cuba ubah kriteria carian atau penapis anda.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    @if($showAssignmentModal)
        <div class="fixed inset-0 z-50 overflow-y-auto">
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                <div class="fixed inset-0 transition-opacity bg-black-500 bg-opacity-75" wire:click="closeAssignmentModal"></div>
                ...
                <div class="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
                    <div>
                        <div class="mx-auto flex items-center justify-center h-12 w-12 rounded-full bg-primary-100">
                            <x-myds.icon name="user-plus" size="24" class="text-primary-600" />
                        </div>
    ...
    @if($showStatusModal)
        <div class="fixed inset-0 z-50 overflow-y-auto">
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                <div class="fixed inset-0 transition-opacity bg-black-500 bg-opacity-75" wire:click="closeStatusModal"></div>
                ...
                <div class="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
                    ...
                    <div class="mt-6 space-y-4">
                        <div>
                            <label class="block text-sm font-medium text-black-700 mb-2">Status Baharu <span class="text-danger-600">*</span></label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-black-700 mb-2">Nota Penyelesaian</label>
                            <textarea
