...
<div class="bg-background-light dark:bg-background-dark min-h-screen">
  <div class="container mx-auto px-4 py-8 max-w-4xl">
    ...
    <x-myds.header>
      <div class="flex flex-col md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <h1
            class="font-poppins text-2xl font-semibold text-black-900 dark:text-white mb-2"
          >
            Tiket #{{ $ticket->ticket_number }}
          </h1>
          <p class="font-inter text-sm text-black-600 dark:text-black-400">
            {{ $ticket->title }}
          </p>
        </div>
        <div class="mt-4 md:mt-0">
          ...
          <span
            class="inline-flex items-center px-3 py-1 rounded-full font-inter text-sm font-medium {{ $statusColorClass }}"
          >
    ...
    >
      <h3
        class="font-poppins text-lg font-medium text-black-900 dark:text-white mb-4"
      >
        Kemajuan Tiket
      </h3>
      <div class="relative">
        <div class="flex items-center justify-between mb-2">
          <span
            class="font-inter text-sm font-medium text-black-700 dark:text-black-300"
          >
            Kemajuan
          </span>
          <span class="font-inter text-sm text-black-500 dark:text-black-400">
            {{ $statusProgress['current'] }} daripada
            {{ $statusProgress['total'] }} langkah
          </span>
        </div>
        ...
        <div
          class="flex justify-between mt-2 font-inter text-xs text-black-500 dark:text-black-400"
        >
          <span>Terbuka</span>
          <span>Diberikan</span>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="lg:col-span-2 space-y-6">
        <!-- Description -->
        <div
          class="bg-white dark:bg-dialog border border-divider rounded-lg p-6"
        >
          <h3
            class="font-poppins text-lg font-medium text-black-900 dark:text-white mb-4"
          >
          ...
          <div class="prose prose-sm max-w-none">
            @if ($showFullDescription || strlen($ticket->description) <= 300)
              <p
                class="font-inter text-sm text-black-700 dark:text-black-300 whitespace-pre-wrap"
              >
                {{ $ticket->description }}
              </p>
            @else
          ...
          >
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              @foreach ($ticket->attachments as $attachment)
                <div
                  class="flex items-center p-3 bg-washed dark:bg-black-100 rounded-lg"
                >
        ...
        @if ($ticket->resolved_at)
          <x-myds.callout variant="success">
            ...
            <div>
              <h4
                class="font-inter text-sm font-medium text-success-700 dark:text-success-500 mb-2"
              >
                Penyelesaian
              </h4>
              @if ($ticket->resolution)
                <p
                  class="font-inter text-sm text-success-700 dark:text-success-500 mb-2"
                >
                  {{ $ticket->resolution }}
                </p>
              @endif
              ...
              <div
                class="font-inter text-sm text-success-600 dark:text-success-400"
              >
                Diselesaikan pada
                {{ $ticket->resolved_at->format('j F Y \p\a\d\a g:i A') }}
      ...
      <div class="space-y-6">
        ...
        >
          ...
          <dl class="space-y-4">
            <div>
              <dt
                class="font-inter text-sm font-medium text-black-500 dark:text-black-400"
              >
                Dicipta
              </dt>
            ...
            <div>
              ...
              <dd>
                ...
                <span
                  class="inline-flex items-center px-2.5 py-0.5 rounded-full font-inter text-xs font-medium {{ $priorityColorClass }}"
                >
            ...
            <div>
              ...
              <dd>
                ...
                <span
                  class="inline-flex items-center px-2.5 py-0.5 rounded-full font-inter text-xs font-medium {{ $urgencyColorClass }}"
                >
                ...
                <dd class="font-inter text-sm text-black-900 dark:text-white">
                  ...
                  @if ($ticket->equipmentItem->serial_number)
                    ...
                    <span class="text-black-500 dark:text-black-400">
                      S/N: {{ $ticket->equipmentItem->serial_number }}
                    </span>
                  @endif
                </dd>
              </div>
            ...
            @if ($ticket->due_at)
              <div>
                ...
                <dd
                  class="font-inter text-sm {{ $ticket->isOverdue() ? 'text-danger-600 font-medium' : 'text-black-900 dark:text-white' }}"
                >
        ...
        <x-myds.callout variant="info">
          ...
          <div>
            <h4
              class="font-inter text-sm font-medium text-primary-700 dark:text-primary-400 mb-2"
            >
              Perjanjian Tahap Perkhidmatan
            </h4>
            <p
              class="font-inter text-sm text-primary-700 dark:text-primary-400 mb-2"
            >
              Sasaran masa respons berdasarkan keutamaan:
              <strong>{{ $ticket->priority->label() }}</strong>
            </p>
            @if ($ticket->response_time)
              <p
                class="font-inter text-sm text-primary-600 dark:text-primary-300"
              >
                Masa respons:
                <strong>
                  {{ number_format($ticket->response_time, 1) }} jam
                </strong>
              </p>
            @endif
            ...
            @if ($ticket->resolution_time)
              ...
              >
                Masa penyelesaian:
                <strong>
                  {{ number_format($ticket->resolution_time, 1) }} jam
                </strong>
              </p>
            @endif
          </div>
