<div class="max-w-4xl mx-auto">
  @if ($submitted)
    <!-- Success State -->
    <div
      class="bg-success-50 dark:bg-success-900/20 border border-success-200 dark:border-success-800 rounded-lg p-6 text-center"
    >
      <div
        class="flex items-center justify-center w-12 h-12 mx-auto mb-4 bg-success-100 dark:bg-success-800 rounded-full"
      >
        <svg
      ...
      <h3
        class="text-lg font-semibold text-success-900 dark:text-success-100 mb-2"
      >
        Damage Report Submitted Successfully
      </h3>
      <p class="text-success-700 dark:text-success-300 mb-4">
        Your damage report has been submitted and a support ticket has been
        created. You will receive an email confirmation shortly.
      </p>
  ...
  @else
    ...
    <div class="mb-6">
      ...
      <p class="text-body-lg text-gray-600 dark:text-gray-400">
        Report damaged ICT equipment and request repair services through this
        secure form.
      </p>
    </div>
    ...
    <div class="mb-8">
      <div class="flex items-center justify-between mb-4">
        @for ($step = 1; $step <= $totalSteps; $step++)
          <div
            class="flex items-center {{ $step < $totalSteps ? 'flex-1' : '' }}"
          >
            <div
              class="flex items-center justify-center w-8 h-8 rounded-full {{ $currentStep >= $step ? 'bg-primary-600 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-400' }}"
            >
              {{ $step }}
            </div>
            @if ($step < $totalSteps)
              <div
                class="flex-1 h-1 mx-4 {{ $currentStep > $step ? 'bg-primary-600' : 'bg-gray-200 dark:bg-gray-700' }}"
              ></div>
            @endif
          </div>
        @endfor
            ...
            <div>
              ...
              <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mt-2">
                @foreach (['low' => 'Low', 'medium' => 'Medium', 'high' => 'High', 'urgent' => 'Urgent'] as $value => $label)
                  <label
                    class="flex items-center p-3 border border-gray-300 dark:border-gray-600 rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 {{ $priority === $value ? 'bg-primary-50 dark:bg-primary-900/20 border-primary-300 dark:border-primary-600' : '' }}"
                  >
                    <input
                      type="radio"
                    ...
                    <div class="flex items-center">
                      <div
                        class="w-4 h-4 border-2 rounded-full mr-3 {{ $priority === $value ? 'border-primary-600 bg-primary-600' : 'border-gray-300 dark:border-gray-600' }}"
                      >
                        @if ($priority === $value)
                          <div
        ...
        @elseif ($currentStep === 3)
          ...
          <div class="space-y-6">
            ...
            <div>
              ...
              <p class="form-help">
                Select a preferred date for repair (optional). We will try to
            ...
            <div class="bg-gray-50 dark:bg-gray-900 rounded-lg p-4">
              <h4 class="text-h5 text-gray-900 dark:text-gray-100 mb-3">
                Review Your Report
              </h4>
              <div class="space-y-2 text-sm">
                ...
                <div class="flex justify-between">
                  ...
                  <span
                    class="px-2 py-1 rounded-full text-xs font-medium {{ $priority === 'urgent' ? 'bg-danger-100 text-danger-800 dark:bg-danger-900/20 dark:text-danger-400' : '' }} {{ $priority === 'high' ? 'bg-warning-100 text-warning-800 dark:bg-warning-900/20 dark:text-warning-400' : '' }} {{ $priority === 'medium' ? 'bg-primary-100 text-primary-800 dark:bg-primary-900/20 dark:text-primary-400' : '' }} {{ $priority === 'low' ? 'bg-gray-100 text-gray-800 dark:bg-gray-900/20 dark:text-gray-400' : '' }}"
                  >
                    {{ ucfirst($priority) }}
                  </span>
                </div>
