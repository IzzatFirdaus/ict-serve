<div>
  ...
  >
    ...
    <div class="mb-6">
      ...
      <div class="flex flex-col lg:flex-row gap-4">
        ...
        <div class="flex-1">
          <x-myds.form.input
            ...
            placeholder="{{ __('forms.placeholders.search_equipment') }}"
            class="w-full"
          />
        </div>
        ...
        >
          ...
          {{ __('equipment_selector.filter') }}
          @if ($selectedCategory || $availabilityFilter !== 'available')
            <span
              class="ml-2 px-2 py-1 bg-primary-100 text-txt-primary rounded-full text-xs"
            >
              Aktif
            </span>
          @endif
      ...
      @if ($showFilters)
        <div
          class="mt-4 p-4 bg-gray-50 rounded-[var(--radius-m)] border border-otl-gray-200"
        >
    ...
    @if (count($selectedItems) > 0)
      <div
        class="mb-6 p-4 bg-primary-50 border border-otl-primary-200 rounded-[var(--radius-m)]"
      >
    ...
    <div class="mb-6">
      @if ($equipment->count() > 0)
        @if ($viewMode === 'grid')
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            @foreach ($equipment as $item)
              <div
                class="border border-otl-gray-200 rounded-[var(--radius-m)] p-4 hover:shadow-md transition-shadow {{ in_array($item->id, $selectedItems) ? 'ring-2 ring-fr-primary bg-primary-50' : '' }}"
              >
                ...
                <div class="space-y-2">
                  ...
                  <div class="flex items-center text-xs">
                    ...
                    <span class="ml-1 text-txt-black-700">
                ...
                @if (in_array($item->id, $selectedItems))
                  <div class="mt-4 pt-3 border-t border-otl-gray-200 space-y-3">
                    ...
                    <div>
                      <label
                        class="block text-xs font-medium text-txt-black-700 mb-1"
                      >
                        Kuantiti
                      </label>
                      <x-myds.form.input
        ...
        @else
          ...
          <div class="space-y-2">
            @foreach ($equipment as $item)
              <div
                class="border border-otl-gray-200 rounded-[var(--radius-s)] p-4 {{ in_array($item->id, $selectedItems) ? 'ring-2 ring-fr-primary bg-primary-50' : '' }}"
              >
      ...
      @else
        ...
        <div class="text-center py-12">
          ...
          />
          <h3 class="text-lg font-medium text-txt-black-900 mb-2">
            {{ __('equipment_selector.no_equipment') }}
          </h3>
          <p class="text-txt-black-600 mb-4">
            @if ($search)
              {{ __('forms.messages.no_equipment_matches', ['search' => $search]) }}
            @else
  ...
  @if (count($selectedEquipmentDetails) > 0)
    <div
      class="mt-6 bg-white rounded-[var(--radius-xl)] border border-otl-gray-200 shadow-md p-6"
    >
      <h4 class="myds-heading-2xs text-txt-black-900 mb-4">
        {{ __('equipment_selector.summary') }}
      </h4>

      <div class="space-y-3">
        @foreach ($selectedEquipmentDetails as $detail)
          <div
            class="flex items-center justify-between p-3 bg-gray-50 rounded-[var(--radius-s)]"
          >
            <div class="flex-1">
