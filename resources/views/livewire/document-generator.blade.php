<div class="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-600 mt-1">
                    Jana dan muat turun dokumen rasmi untuk permohonan ini
                </p>
            </div>
            @if($loanRequest)
    ...
    <div class="px-6 py-6">
        <h4 class="text-sm font-medium text-gray-900 mb-4">Pilih Jenis Dokumen</h4>
        ...
        <div class="px-6 py-4 bg-gray-50 border-t border-gray-200">
            <div class="flex flex-wrap gap-3">
                ...
                class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-txt-white bg-bg-primary-600 hover:bg-bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-fr-primary disabled:opacity-50 disabled:cursor-not-allowed">
                    ...
                    <div wire:loading wire:target="generatePDF" class="flex items-center">
    ...
    @if($showPreview && $loanRequest)
        <div class="px-6 py-6 border-t border-gray-200">
            <h4 class="text-sm font-medium text-gray-900 mb-4">Pratonton Dokumen</h4>
            <div class="bg-gray-50 border border-gray-200 rounded-lg p-6">
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <p class="mt-1 text-sm text-gray-900">{{ $loanRequest->user->name ?? 'N/A' }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700">No. Rujukan</label>
                            <p class="mt-1 text-sm text-gray-900">{{ $loanRequest->reference_number ?? 'N/A' }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Tarikh Permohonan</label>
                            <p class="mt-1 text-sm text-gray-900">{{ $loanRequest->created_at?->format('d/m/Y') ?? 'N/A' }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Status</label>
                            <p class="mt-1 text-sm text-gray-900">{{ ucfirst($loanRequest->status) }}</p>
                        </div>
                    </div>
                    ...
                    @if($loanRequest->equipmentItems && $loanRequest->equipmentItems->count() > 0)
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Peralatan yang Dipohon</label>
                            <div class="bg-white border border-gray-200 rounded-md overflow-hidden">
                                <table class="min-w-full divide-y divide-gray-200">
                                    <thead class="bg-gray-50">
                                        <tr>
                                            <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Peralatan</th>
                                            <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Kuantiti</th>
                                            <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Tempoh</th>
                                        </tr>
                                    </thead>
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        @foreach($loanRequest->equipmentItems as $item)
                                            <tr>
                                                ...
                                                <td class="px-4 py-2 text-sm text-gray-900">{{ $item->pivot->quantity ?? 1 }}</td>
                    ...
                    @if($documentType === 'approval_letter' && $loanRequest->status === 'approved')
                        <div class="mt-6 p-4 bg-success-50 border border-success-200 rounded-md">
                            <h5 class="text-sm font-medium text-success-800">Keputusan Permohonan</h5>
                            <p class="text-sm text-success-700 mt-1">
                                Permohonan ini telah diluluskan pada {{ $loanRequest->approved_at?->format('d/m/Y H:i') }}.
                            </p>
                        </div>
                    @endif
                </div>
                ...
                <div class="mt-6 text-center">
                    <p class="text-xs text-gray-500">
                        Ini adalah pratonton ringkas. Dokumen sebenar akan mengandungi maklumat yang lebih lengkap.
                    </p>
                </div>
            </div>
        </div>
    ...
    @if($availableDocuments->count() > 0)
        <div class="px-6 py-6 border-t border-gray-200">
            <h4 class="text-sm font-medium text-gray-900 mb-4">Dokumen Sedia Ada</h4>
            <div class="space-y-3">
