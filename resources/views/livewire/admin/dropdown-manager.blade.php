{{--
    ICTServe (iServe) – Dropdown (Damage Type) Manager
...
<div class="myds-container myds-py-6 bg-washed min-h-screen">
    <!-- Breadcrumb Navigation -->
    <nav aria-label="Breadcrumb" class="mb-4">
        <ol class="myds-breadcrumb">
            <li class="myds-breadcrumb-item">
                <a href="{{ route('dashboard') }}" class="myds-link">Papan Pemuka / Dashboard</a>
            </li>
            <li class="myds-breadcrumb-item">
    ...
    <!-- Page Header -->
    <header class="mb-6">
        <h1 class="myds-heading-xl text-myds-gray-900 dark:text-myds-gray-100 flex items-center gap-3">
            <i class="myds-icon-cog" aria-hidden="true"></i>
            Pengurusan Jenis Kerosakan / Damage Type Management
        </h1>
    ...
    @if (session()->has('success'))
        <div class="myds-callout myds-callout-success mb-6" role="status" aria-live="polite">
            <i class="myds-icon-check-circle" aria-hidden="true"></i>
            <span>{{ session('success') }}</span>
        </div>
    @endif
    @if (session()->has('error'))
        <div class="myds-callout myds-callout-error mb-6" role="alert" aria-live="assertive">
    ...
    <section class="myds-card myds-card-bordered mb-6">
        <div class="myds-card-body">
            <div class="myds-flex myds-justify-between myds-items-center myds-gap-4 flex-wrap">
                <div class="myds-flex myds-items-center myds-gap-4 flex-1 min-w-0">
                    <!-- Search -->
                    <div class="flex-1 max-w-md min-w-[180px]">
                        ...
                        <div class="myds-input-group">
                            ...
                            <input
                                ...
                                autocomplete="off"
                                aria-label="Cari jenis kerosakan / Search damage types"
                            />
                        </div>
                    </div>
                    <!-- Severity Filter -->
                    <div>
                        <label for="severityFilter" class="sr-only">Tapis tahap / Filter severity</label>
                        <select id="severityFilter" wire:model.live="severityFilter" class="myds-select">
                    ...
                    <!-- Status Filter -->
                    <div>
                        <label for="statusFilter" class="sr-only">Tapis status / Filter status</label>
                        <select id="statusFilter" wire:model.live="statusFilter" class="myds-select">
                ...
                <button type="button"
                    ...
                    aria-label="Tambah Jenis Kerosakan / Add Damage Type"
                >
    ...
    <!-- Form Modal/Card -->
    @if ($showForm)
        <section class="myds-card myds-card-elevated mb-6" aria-modal="true" aria-labelledby="damage-type-form-title">
            <header class="myds-card-header">
                <h2 id="damage-type-form-title" class="myds-heading-lg text-myds-gray-900 dark:text-myds-gray-100 flex items-center gap-2">
                    <i class="myds-icon-plus-circle" aria-hidden="true"></i>
                    {{ $editingId ? 'Kemaskini Jenis Kerosakan / Update Damage Type' : 'Tambah Jenis Kerosakan Baru / Add New Damage Type' }}
                </h2>
            </header>
            <form wire:submit.prevent="save" class="myds-card-body myds-form-grid" novalidate autocomplete="off">
                <!-- Name (English) -->
                <div>
                ...
                <div>
                    ...
                    <input type="text" id="name_bm" wire:model="name_bm"
                ...
                <div>
                    ...
                    <input type="number" id="sort_order" wire:model="sort_order"
                        ...
                        min="0" required
                        aria-describedby="sort_order_help"
                    />
                    <p id="sort_order_help" class="myds-help-text">
                ...
                <div>
                    ...
                    <input type="color" id="color_code" wire:model="color_code"
                ...
                <div class="myds-col-span-full">
                    ...
                    <textarea id="description_bm" wire:model="description_bm"
                ...
                <!-- Form Actions -->
                <div class="myds-flex myds-justify-end myds-gap-4 mt-6 pt-6 border-t border-myds-gray-200">
                    <button type="button" wire:click="cancelEdit" class="myds-button myds-button-secondary">
                        <i class="myds-icon-x mr-2" aria-hidden="true"></i>
                        Batal / Cancel
                    </button>
                    <button type="submit" class="myds-button myds-button-primary"
                        wire:loading.attr="disabled" wire:target="save"
                        aria-label="{{ $editingId ? 'Kemaskini / Update' : 'Simpan / Save' }}"
                    >
                        <span wire:loading.remove wire:target="save">
                            <i class="myds-icon-check mr-2" aria-hidden="true"></i>
                            {{ $editingId ? 'Kemaskini / Update' : 'Simpan / Save' }}
                        </span>
                ...
                @error('save')
                    <div class="myds-callout myds-callout-error mt-4" role="alert">
                        <i class="myds-icon-exclamation-circle" aria-hidden="true"></i>
                        <span>{{ $message }}</span>
                    </div>
                @enderror
    ...
    <!-- Data Table -->
    <section class="myds-card myds-card-bordered" aria-labelledby="damage-type-list-title">
        <header class="myds-card-header">
            <h2 id="damage-type-list-title" class="myds-heading-lg text-myds-gray-900 dark:text-myds-gray-100 flex items-center gap-2">
                <i class="myds-icon-view-list" aria-hidden="true"></i>
        ...
        <div class="myds-card-body p-0">
            @if ($damageTypes->count())
                <div class="myds-table-responsive">
                    <table class="myds-table" role="table" aria-describedby="damage-types-table-caption">
                        <caption id="damage-types-table-caption" class="sr-only">
                            List of damage types with order, name, severity, status, updated date, and actions
                        </caption>
                            ...
                            @foreach ($damageTypes as $damageType)
                                <tr class="myds-table-row">
                                    <td class="myds-table-td">
                                        <span class="myds-badge myds-badge-neutral">{{ $damageType->sort_order }}</span>
                                    </td>
                                    <td class="myds-table-td">
                                        <div class="myds-flex myds-items-center">
                                            @if ($damageType->icon)
                                            ...
                                            @endif
                                            <div>
                                                <div class="myds-text-body-md font-medium">{{ $damageType->name }}</div>
                                                <div class="myds-text-body-sm text-myds-gray-600">{{ $damageType->name_bm }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="myds-table-td">
                                    ...
                                    <td class="myds-table-td">
                                        <div class="myds-flex myds-gap-2">
                                            ...
                                            <button type="button"
                                                ...
                                                aria-label="{{ $damageType->is_active ? 'Nyahaktifkan / Deactivate' : 'Aktifkan / Activate' }}"
                                            >
            ...
            @else
                <div class="myds-card-body text-center py-16">
                    ...
                    <p class="myds-text-body-md text-myds-gray-600 dark:text-myds-gray-400">
                        @if ($search || $severityFilter || $statusFilter)
                            Tiada hasil sepadan dengan carian anda. Cuba ubah kriteria carian.<br>
                            No results match your search. Try adjusting your search criteria.
                        @else
                            Tambah jenis kerosakan pertama untuk memulakan.<br>
                    ...
                    @if (!$search && !$severityFilter && !$statusFilter)
                        <button type="button"
                            ...
                            aria-label="Tambah Jenis Kerosakan Pertama / Add First Damage Type"
                        >
                            <i class="myds-icon-plus mr-2" aria-hidden="true"></i>
