<div
  class="bg-white border border-divider rounded-lg p-4"
  x-data="{
...
>
  ...
  <div class="flex items-center justify-between mb-4">
    ...
    <div class="flex items-center space-x-2">
      ...
      <div class="flex items-center space-x-2">
        ...
        >
          <span
            class="pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out {{ $autoRefresh ? 'translate-x-5' : 'translate-x-0' }}"
          ></span>
        </button>
      </div>
  ...
  <div class="space-y-3">
    @forelse ($items as $item)
      <div
        class="flex items-center justify-between p-3 bg-washed rounded-lg"
        ...
        <div class="flex items-center space-x-3">
          <div class="flex-shrink-0">
            @if ($item['status'] === 'active')
              <div
                class="w-3 h-3 bg-success-500 rounded-full animate-pulse"
              ></div>
            @elseif ($item['status'] === 'warning')
              <div
                class="w-3 h-3 bg-warning-500 rounded-full animate-pulse"
              ></div>
            @elseif ($item['status'] === 'error')
              <div
                class="w-3 h-3 bg-danger-500 rounded-full animate-pulse"
              ></div>
            @else
          ...
          <div>
            <div class="text-sm font-medium text-black-900 font-inter">
              {{ $item['label'] }}
            </div>
            <div class="text-xs text-black-500 font-inter">
              {{ $item['value'] }}
            </div>
          </div>
        </div>
    ...
    <div class="mt-2 flex items-center justify-center">
      <div class="flex items-center space-x-2 text-xs text-primary-600">
        ...
        <span class="font-inter">
          Dimuat semula automatik setiap {{ $refreshInterval }} saat
        </span>
      </div>
    </div>
  @endif
