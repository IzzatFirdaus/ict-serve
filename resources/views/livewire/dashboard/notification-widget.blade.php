...
<div class="bg-white rounded-lg border border-divider p-6">
  ...
  @if (count($notifications) > 0)
    <div class="space-y-3">
      @foreach ($notifications as $notification)
        <div
          class="p-3 border border-divider rounded-md {{ $notification['type'] === 'success' ? 'bg-success-50 border-success-200' : 'bg-background-light' }}"
        >
          <div class="flex items-start gap-3">
            @if ($notification['type'] === 'success')
              <svg
                class="w-4 h-4 text-success-600 flex-shrink-0 mt-0.5"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
            ...
            <div class="flex-1">
              ...
              <p class="font-inter text-xs text-black-700">
  ...
  @else
    <div class="text-center py-6">
      <svg
        class="w-6 h-6 text-black-400 mx-auto"
        fill="none"
        stroke="currentColor"
        viewBox="0 0 24 24"
      >
      ...
      <p class="font-inter text-sm text-black-500 mt-2">
