<x-myds.container variant="wide" class="min-h-screen">
  ...
  <div class="mt-6">
    <x-myds.tabs
      :tabs="collect($tabs)->map(function($tab, $key) {
        return [
            'key' => $key,
            'label' => $tab['title'],
            'icon' => $tab['icon'] ?? null,
  ...
  <div class="py-6">
          ...
          <x-myds.card>
            <div class="flex items-center">
              ...
              <div class="ml-4 flex-1">
                ...
                <x-myds.heading level="2">
                  {{ number_format($stats['available_equipment']) }}/{{ number_format($stats['total_equipment']) }}
                </x-myds.heading>
              </div>
            </div>
          </x-myds.card>
          ...
          @foreach ($recentActivity as $activity)
            <div class="flex items-start space-x-3">
              ...
              <div class="min-w-0 flex-1">
                ...
                <x-myds.text size="xs" variant="muted" class="mt-1">
          ...
          <div class="flex items-center justify-between">
            ...
            @if ($showBulkActions)
              <div class="flex items-center space-x-2">
                <x-myds.text size="sm">
                  {{ count($selectedRequests) }} dipilih
                </x-myds.text>
                <x-myds.button
                  variant="success"
                  size="sm"
          ...
          <tbody>
            @foreach ($loanRequests as $request)
              <tr class="hover:bg-gray-50">
                ...
                <td class="px-6 py-4">
                  <x-myds.text class="max-w-xs truncate">
                    {{ $request->purpose ?? 'N/A' }}
                  </x-myds.text>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="flex items-center space-x-2">
                    <x-myds.button variant="ghost" size="sm">
                      Lihat
                    </x-myds.button>
                    <x-myds.button variant="ghost" size="sm">
                      Edit
      ...
      <x-myds.card>
        <div class="p-4 border-b border-otl-divider">
          <div class="flex items-center justify-between">
            ...
            <x-myds.button icon="➕">Tambah Pengguna</x-myds.button>
          </div>
        </div>

        <div class="p-6">
          <x-myds.text variant="muted">
            Senarai pengguna akan dipaparkan di sini.
          </x-myds.text>
        </div>
      </x-myds.card>
    @endif
    ...
    @if ($activeTab === 'reports')
      <x-myds.card>
        ...
        <div class="p-6">
          <x-myds.text variant="muted">
            Laporan dan carta akan dipaparkan di sini.
          </x-myds.text>
        </div>
      </x-myds.card>
    @endif
