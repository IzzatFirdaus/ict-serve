<div class="fixed top-4 right-4 z-50 space-y-4 pointer-events-none">
    @foreach($toasts as $toast)
        ...
        >
            ...
            <div class="bg-white {{ $toastConfig['border'] }} border rounded-lg shadow-lg p-4">
                <div class="flex">
                    ...
                    <div class="ml-3 w-0 flex-1">
                        @if($toast['title'])
                            <p class="text-sm font-medium {{ $toastConfig['titleColor'] }} font-inter">
                                {{ $toast['title'] }}
                            </p>
                        @endif
                        <p class="text-sm {{ $toastConfig['messageColor'] }} font-inter {{ $toast['title'] ? 'mt-1' : '' }}">
                            {{ $toast['message'] }}
                        </p>
                    </div>
