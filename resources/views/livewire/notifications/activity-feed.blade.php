<div class="bg-white border border-divider rounded-lg">
  ...
  <div class="px-4 py-3 border-b border-divider">
    <div class="flex items-center justify-between">
      <h3 class="text-lg font-medium text-black-900 font-poppins">
        Log Aktiviti
      </h3>
  ...
  @if ($showFilters)
    <div class="px-4 py-3 bg-washed border-b border-divider">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div>
          <label
            class="block text-xs font-medium text-black-700 mb-2 font-inter"
          >
            Jenis Aktiviti
        ...
        <div>
          ...
          <x-myds.input type="date" wire:model.live="dateFrom" class="w-full" />
        </div>
        ...
        <div>
          ...
          <x-myds.input type="date" wire:model.live="dateTo" class="w-full" />
        </div>
      </div>
  ...
  <div class="divide-y divide-divider">
    @forelse ($activities as $activity)
      <div
        class="px-4 py-3 hover:bg-washed transition-colors duration-150"
      ...
      >
        <div class="flex items-start space-x-3">
          ...
          <div class="flex-shrink-0">
            <div
              class="w-8 h-8 rounded-full {{ $this->getActivityColor($activity->log_name) === 'primary' ? 'bg-primary-100' : ($this->getActivityColor($activity->log_name) === 'success' ? 'bg-success-100' : ($this->getActivityColor($activity->log_name) === 'warning' ? 'bg-warning-100' : 'bg-black-100')) }} flex items-center justify-center"
            >
              <x-myds.icon
          ...
          <div class="flex-1 min-w-0">
            ...
            @if ($activity->properties && $activity->properties->count() > 0)
              <div class="mt-2">
                <details class="group">
                  <summary
                    class="text-xs text-primary-600 cursor-pointer hover:text-primary-700 font-inter"
                  >
                    Lihat Butiran
                  </summary>
                  <div
                    class="mt-2 text-xs text-black-500 bg-black-50 rounded p-2 font-inter"
                  >
    ...
    @empty
      <div class="px-4 py-8 text-center">
        <x-myds.icon
          name="document"
          size="32"
          class="mx-auto text-black-300 mb-2"
        />
