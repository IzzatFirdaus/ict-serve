...
@props([
  'value' => null,
  'required' => false,
  //Optionallyallowpassingacustomidfor'for'attributeinparentusage,
])

<label
  {{ $attributes->merge(['class' => 'block mb-1 text-sm font-medium text-txt-black-900']) }}
>
  @if ($value)
    {{ $value }}
  @else
  ...
  @if ($required)
    <span
      class="text-txt-danger ml-1 align-middle"
      aria-label="required"
      title="Required"
    >
      *
