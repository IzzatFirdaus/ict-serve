...
<div class="loan-detail-view max-w-6xl mx-auto space-y-8">
  ...
  <div class="bg-bg-white rounded-lg shadow-card border border-otl-divider p-6">
    <div class="flex items-start justify-between">
      <div>
        <h1 class="text-2xl font-bold text-txt-black-900 font-poppins">
          {{ $loanRequest->request_number }}
        </h1>
        <p class="text-sm text-txt-black-500 mt-1">
          Submitted on
          {{ $loanRequest->created_at->format('F j, Y \a\t g:i A') }}
        </p>
      </div>
          ...
          >
            <div class="py-1">
              @if ($loanRequest->canBeEdited())
                <a
                  href="#"
                  class="block px-4 py-2 text-sm text-txt-black-700 hover:bg-bg-gray-50 focus:bg-bg-gray-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-fr-primary rounded-md transition-colors duration-100"
                  role="menuitem"
                  tabindex="0"
                >
              ...
              @if ($loanRequest->canBeCancelled())
                ...
                <a
                  href="#"
                  class="block px-4 py-2 text-sm text-txt-danger hover:bg-warning-50 focus:bg-warning-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-fr-danger rounded-md transition-colors duration-100"
                  role="menuitem"
                  tabindex="0"
                >
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-2 space-y-6">
      <!-- Request Information -->
      <div
        class="bg-bg-white rounded-lg shadow-card border border-otl-divider p-6"
      >
        <h3 class="text-lg font-semibold text-txt-black-900 mb-4 font-poppins">
          Request Information
        </h3>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label class="text-sm font-medium text-txt-black-500">
              Purpose
            </label>
          ...
          <div>
            ...
            <p class="text-sm text-txt-black-900 mt-1">
              ...
              <span class="text-txt-black-500">
                ({{ $loanRequest->loan_duration }} days)
              </span>
            </p>
          </div>
      ...
      @if ($loanRequest->equipment_requests)
        ...
        >
          <h3
            class="text-lg font-semibold text-txt-black-900 mb-4 font-poppins"
          >
            Equipment Requested
          </h3>
          <div class="space-y-4">
            @foreach ($loanRequest->equipment_requests as $equipment)
              <div
                class="flex items-center justify-between p-4 bg-bg-gray-50 rounded-lg"
              >
                ...
                <div class="text-right">
                  <span class="text-sm font-medium text-txt-black-900">
        ...
        >
          ...
          <div class="space-y-4">
            @foreach ($loanRequest->loanItems as $item)
              <div
                class="flex items-center justify-between p-4 border border-otl-divider rounded-lg"
              >
                <div class="flex items-center space-x-4">
                  <div
                  ...
                  <div>
                    ...
                    <p class="text-xs text-txt-black-500">
                      Serial: {{ $item->equipmentItem->serial_number }}
                    </p>
                  </div>
                </div>
                <div class="text-right">
                  ...
                  @if ($item->condition_out)
                    <p class="text-xs text-txt-black-500">
                      Condition: {{ ucfirst($item->condition_out) }}
                    </p>
                  @endif
                </div>
              </div>
        ...
        >
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            @if ($loanRequest->pickup_signature_path)
              <div>
                <h4 class="text-sm font-medium text-txt-black-900 mb-2">
                  Pickup Signature
                </h4>
                <div
                  class="border border-otl-divider rounded-lg p-4 bg-bg-gray-50"
                >
                  <img
                    src="{{ Storage::url($loanRequest->pickup_signature_path) }}"
                ...
                <p class="text-xs text-txt-black-500 mt-2">
                  Signed on
                  {{ $loanRequest->issued_at->format('M j, Y \a\t g:i A') }}
                </p>
              </div>
            @endif
            ...
            @if ($loanRequest->return_signature_path)
              <div>
                ...
                >
                  <img
                    src="{{ Storage::url($loanRequest->return_signature_path) }}"
                ...
                <p class="text-xs text-txt-black-500 mt-2">
                  Signed on
                  {{ $loanRequest->returned_at->format('M j, Y \a\t g:i A') }}
                </p>
              </div>
            @endif
          </div>
        ...
        <div class="space-y-3 text-sm">
          @if ($loanRequest->user->division)
            <div class="flex justify-between">
              ...
              <span class="text-txt-black-900">
                {{ $loanRequest->user->division }}
              </span>
            </div>
          @endif
        ...
        >
          ...
          <div class="space-y-3 text-sm">
            <div class="flex justify-between">
              ...
              <span class="text-txt-black-900">
                {{ $loanRequest->responsible_officer_name }}
              </span>
            </div>
            @if ($loanRequest->responsible_officer_position)
              <div class="flex justify-between">
                <span class="text-txt-black-500">Position:</span>
                <span class="text-txt-black-900">
                  {{ $loanRequest->responsible_officer_position }}
                </span>
              </div>
            @endif
            ...
            @if ($loanRequest->responsible_officer_phone)
              <div class="flex justify-between">
                <span class="text-txt-black-500">Phone:</span>
                <span class="text-txt-black-900">
                  {{ $loanRequest->responsible_officer_phone }}
                </span>
              </div>
            @endif
          </div>
            ...
            <div class="flex-1">
              ...
              @if ($loanRequest->supervisor_approved_at)
                ...
                @if ($loanRequest->supervisor_notes)
                  <p class="text-xs text-txt-black-500 mt-1 italic">
                    {{ $loanRequest->supervisor_notes }}
                  </p>
                @endif
              @else
        ...
        <div class="space-y-3">
          ...
          @if ($loanRequest->supervisor_approved_at)
            <div class="flex justify-between text-sm">
              ...
              <span class="text-txt-black-900">
                {{ $loanRequest->supervisor_approved_at->diffForHumans() }}
              </span>
            </div>
          @endif
          ...
          @if ($loanRequest->ict_approved_at)
            <div class="flex justify-between text-sm">
              ...
              <span class="text-txt-black-900">
                {{ $loanRequest->ict_approved_at->diffForHumans() }}
              </span>
            </div>
          @endif
          ...
          @if ($loanRequest->issued_at)
            <div class="flex justify-between text-sm">
              ...
              <span class="text-txt-black-900">
                {{ $loanRequest->issued_at->diffForHumans() }}
              </span>
            </div>
          @endif
          ...
          @if ($loanRequest->returned_at)
            <div class="flex justify-between text-sm">
              ...
              <span class="text-txt-black-900">
                {{ $loanRequest->returned_at->diffForHumans() }}
              </span>
            </div>
          @endif
        </div>
