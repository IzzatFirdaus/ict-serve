      ...
      @foreach ($processSteps as $index => $step)
        ...
        <div class="flex items-center {{ ! $isLast ? 'flex-1' : '' }}">
          ...
          <div class="relative flex items-center justify-center">
            <div
              class="flex items-center justify-center {{ $compact ? 'w-8 h-8' : 'w-10 h-10' }} rounded-full border-2 {{ $classes['border'] }} {{ $classes['bg'] }}"
              aria-hidden="true"
            >
              @if ($status === 'completed')
                <svg
                  class="w-5 h-5 {{ $classes['text'] }}"
                  viewBox="0 0 20 20"
                  fill="currentColor"
                  aria-hidden="true"
                >
              ...
              @else
                <span
                  class="block {{ $compact ? 'w-2.5 h-2.5' : 'w-3 h-3' }} rounded-full {{ $classes['bg'] === 'bg-gray-200' ? 'bg-gray-400' : $classes['bg'] }}"
                ></span>
              @endif
            </div>
          </div>
          ...
          <div class="ml-4 min-w-0 {{ ! $isLast ? 'flex-1' : '' }}">
            <div
              class="text-sm font-medium {{ $isCurrent ? 'text-txt-primary' : ($status === 'completed' ? 'text-txt-success' : 'text-txt-black-500') }}"
          ...
          @if (! $isLast)
            <div class="flex-1 ml-4">
              <div
                class="{{ $compact ? 'h-0.5' : 'h-1' }} {{ $status === 'completed' ? 'bg-success-600' : 'bg-gray-200' }}"
                aria-hidden="true"
              ></div>
            </div>
          @endif
        ...
        @foreach ($processSteps as $index => $step)
          ...
          <li role="listitem">
            <div class="relative pb-8">
              ...
              @if (! $isLast)
                <span
                  class="absolute top-4 left-4 -ml-px h-full w-0.5 {{ $status === 'completed' ? 'bg-success-600' : 'bg-gray-200' }}"
                  aria-hidden="true"
                ></span>
              @endif
              ...
              <div class="relative flex space-x-3">
                ...
                <div>
                  <span
                    class="h-8 w-8 rounded-full {{ $classes['bg'] }} border-2 {{ $classes['border'] }} flex items-center justify-center ring-8 ring-bg-white"
                    aria-hidden="true"
                  >
                    @if ($status === 'completed')
                      <svg
                    ...
                    @else
                      <span
                        class="block w-2.5 h-2.5 rounded-full {{ $classes['bg'] === 'bg-gray-200' ? 'bg-gray-400' : $classes['bg'] }}"
                      ></span>
                    @endif
                  </span>
                </div>
    ...
    >
      <div class="flex items-center justify-between">
        ...
        <div class="text-right">
          <div class="text-sm font-medium text-txt-black-900">Kemajuan</div>
