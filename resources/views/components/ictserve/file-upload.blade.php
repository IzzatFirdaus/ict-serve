...
{{-- JavaScript moved to resources/js/components/file-upload.js and exposed via resources/js/app.js (@vite) --}}
  ...
  >
    ...
    >
      <path
        ...
        d="M14 10v5a3 3 0 01-3 3H6a3 3 0 01-3-3v-3m10-6l-4-4-4 4m4-4v10"
      />
    </svg>
  ...
  <div x-show="files.length > 0" class="mt-4 space-y-3">
    <template x-for="(file, index) in files" :key="file.id">
      <div
        class="flex items-center p-3 bg-gray-50 rounded-lg border border-otl-gray-200"
      >
        {{-- File Icon --}}
        <div class="flex-shrink-0 mr-3">
          ...
          >
            <span
              ...
              x-text="getFileExtension(file.name)"
            ></span>
          </div>
        </div>
        ...
        <div class="flex-1 min-w-0">
          <div class="flex items-center justify-between">
            <p
              class="text-sm font-medium text-txt-black-900 truncate"
              x-text="file.name"
            ></p>
            <p
              class="text-sm text-txt-black-500 ml-2"
          ...
          {{-- Upload Status --}}
          <div
            x-show="file.uploaded"
            class="mt-1 flex items-center text-xs text-txt-success"
          >
            <svg
              class="w-4 h-4 mr-1"
              viewBox="0 0 20 20"
          ...
          <div
            x-show="file.error"
            class="mt-1 text-xs text-txt-danger"
            x-text="file.error"
            role="alert"
          ></div>
        </div>
        ...
        <div
          x-show="isImage(file) && showPreview"
  ...
  <div x-show="existingFiles.length > 0" class="mt-4">
    ...
    <div class="space-y-2">
      <template x-for="file in existingFiles" :key="file.id">
        <div
          class="flex items-center justify-between p-2 bg-primary-50 rounded border border-otl-primary-200"
        >
          <div class="flex items-center">
            <svg
              class="w-4 h-4 text-txt-primary mr-2"
              viewBox="0 0 20 20"
              fill="currentColor"
              aria-hidden="true"
            >
              <path
                d="M4 3h7l5 5v7a2 2 0 01-2 2H4a2 2 0 01-2-2V5a2 2 0 012-2z"
              />
            </svg>
  ...
  {{-- File Limit Warning --}}
  <div
    x-show="files.length >= maxFiles && multiple"
    class="mt-2 text-xs text-txt-warning"
  >
...
>
  ...
  >
    ...
    <div
      class="inline-block align-bottom bg-bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full"
    >
      <div class="bg-bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
        <div class="flex items-center justify-between mb-4">
          <h3
            id="file-preview-title"
            class="text-lg font-medium text-txt-black-900"
          >
            Pratonton Fail
          </h3>
          <button
            @click="showPreviewModal = false"
            class="text-txt-black-400 hover:text-txt-black-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-fr-primary rounded"
        ...
        <div class="text-center">
          ...
          <div x-show="previewFile && ! isImage(previewFile)" class="py-8">
            <svg
              class="mx-auto h-16 w-16 text-txt-black-400"
              viewBox="0 0 20 20"
              fill="none"
              stroke="currentColor"
              stroke-width="1.5"
            ...
            >
              <path
                stroke-linecap="round"
                stroke-linejoin="round"
                d="M6 4h5l4 4v8a2 2 0 01-2 2H6a2 2 0 01-2-2V6a2 2 0 012-2z"
              />
            </svg>
          ...
          <p
            class="mt-2 text-sm text-txt-black-700"
            x-text="previewFile ? previewFile.name : ''"
          ></p>
        </div>
      </div>
    </div>
...
@push('scripts')
@vite('resources/js/components/file-upload-alpine.js')
@endpush
