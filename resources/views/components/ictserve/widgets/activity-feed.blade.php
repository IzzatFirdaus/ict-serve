...
<div class="bg-white dark:bg-dialog shadow-card rounded-lg border border-otl-divider" {{ $attributes }} role="region" aria-labelledby="activity-feed-title">
    {{-- Header --}}
    @if($showHeader)
        <header class="px-6 py-4 border-b border-otl-divider">
            <div class="flex items-center justify-between">
                <h3 id="activity-feed-title" class="text-lg font-poppins font-semibold text-txt-black-900">
                    {{ $title }}
                </h3>
            ...
            <div class="space-y-4" aria-busy="true" aria-live="polite">
                @for($i = 0; $i < 3; $i++)
                    <div class="animate-pulse flex items-start space-x-3">
                        <div class="h-8 w-8 bg-gray-200 dark:bg-gray-700 rounded-full" aria-hidden="true"></div>
                        <div class="flex-1 space-y-2">
                            <div class="h-4 bg-gray-200 dark:bg-gray-700 rounded w-3/4" aria-hidden="true"></div>
                            <div class="h-3 bg-gray-200 dark:bg-gray-700 rounded w-1/2" aria-hidden="true"></div>
                        </div>
                    </div>
                @endfor
            </div>
        @elseif(count($activities) > 0)
                ...
                @foreach($activities->take($maxItems) as $activity)
                    <li role="listitem">
                        <div class="relative pb-8">
                            {{-- Timeline Line --}}
                            @if(!$loop->last)
                                <span class="absolute top-4 left-4 -ml-px h-full w-0.5 bg-otl-divider" aria-hidden="true"></span>
                            @endif
                            ...
                            <div class="relative flex items-start space-x-3">
                                ...
                                <div class="relative flex-shrink-0">
                                    ...
                                    <span class="{{ $iconSizeClass }} {{ $iconBg }}" aria-hidden="true">
                                        ...
                                        @else
                                            ...
                                            @switch($activityType)
                                                @case('success')
                                                    <svg class="h-5 w-5 text-white" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.5" xmlns="http://www.w3.org/2000/svg" focusable="false" aria-hidden="true">
                                                ...
                                                @case('warning')
                                                    <svg class="h-5 w-5 text-white" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.5" xmlns="http://www.w3.org/2000/svg" focusable="false" aria-hidden="true">
                                                        <path stroke-linecap="round" stroke-linejoin="round" d="M10 3.5L2.4 17h15.2L10 3.5zM10 9v2m0 2h.01"/>
                                                    </svg>
                                                    @break
                                                ...
                                                @default
                                                    <svg class="h-5 w-5 text-white" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.5" xmlns="http://www.w3.org/2000/svg" focusable="false" aria-hidden="true">
                                                        <path stroke-linecap="round" stroke-linejoin="round" d="M10 2a8 8 0 100 16 8 8 0 000-16zM11 7h-2v5h2V7zM11 14h-2v1h2v-1z"/>
                                                    </svg>
                                            @endswitch
                                        @endif
                                    </span>
                                ...
                                <div class="min-w-0 flex-1">
                                    <div>
                                        ...
                                        {{-- Activity Meta --}}
                                        <div class="mt-2 flex flex-wrap items-center gap-2 text-sm text-txt-black-500">
                                            @if(isset($activity['user']))
                                                <span aria-label="Pengguna">{{ $activity['user'] }}</span>
                                                <span aria-hidden="true">&middot;</span>
                                            @endif

                                            @if(isset($activity['timestamp']))
                                                <time datetime="{{ is_object($activity['timestamp']) && method_exists($activity['timestamp'], 'toIso8601String') ? $activity['timestamp']->toIso8601String() : $activity['timestamp'] }}"
                                        ...
                                        @if(isset($activity['url']))
                                            <div class="mt-2">
                                                ...
                                                >
                                                    {{ $activity['action_text'] ?? 'Lihat Detail' }} <span aria-hidden="true">→</span>
                                                </a>
                                            </div>
                                        @endif
                                    </div>
            ...
            @if($showViewAll && $viewAllUrl && count($activities) > $maxItems)
                <div class="mt-4 pt-4 border-t border-otl-divider">
                    <a href="{{ $viewAllUrl }}" class="text-sm font-medium text-txt-primary hover:text-primary-700 focus-visible:ring-2 focus-visible:ring-fr-primary rounded">
        ...
        @else
            ...
            <div class="text-center py-8">
                <div class="mx-auto h-12 w-12 text-txt-black-400" aria-hidden="true">
                    <svg class="h-12 w-12 mx-auto text-otl-divider" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.5" xmlns="http://www.w3.org/2000/svg">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4l3 3M18 12a6 6 0 11-12 0 6 6 0 0112 0z"/>
                    </svg>
                </div>
                <h4 class="mt-2 text-sm font-medium text-txt-black-900">Tiada aktiviti</h4>
