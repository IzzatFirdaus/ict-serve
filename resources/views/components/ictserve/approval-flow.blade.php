...
>
  @if ($orientation === 'horizontal')
    ...
    <div
      class="flex items-center {{ $compact ? 'gap-2' : 'gap-6' }} overflow-x-auto"
      tabindex="0"
    >
        ...
        <div class="flex items-center w-full min-w-0">
            ...
            @if ($showLabels && ! $compact)
              <div class="mt-2 text-center min-w-[100px] max-w-[180px]">
                ...
                @if (isset($step['description']))
                  <p
                    class="text-xs text-txt-black-500 mt-1 truncate"
          ...
          @if (! $isLast)
            ...
            <div class="flex-1 flex items-center">
              <div
                class="h-0.5 w-full mx-2 {{ $lineColor }} transition-colors duration-300"
              ></div>
            </div>
          @endif
        </div>
      ...
      <div class="mt-4 grid grid-cols-{{ count($steps) }} gap-2">
        @foreach ($steps as $index => $step)
          <div class="text-center min-w-0">
            <p
              class="text-xs font-medium text-txt-black-900 truncate"
              title="{{ $step['title'] ?? 'Langkah ' . ($index + 1) }}"
            >
              {{ $step['title'] ?? __('Langkah ') . ($index + 1) }}
            </p>
    ...
    <div class="flex flex-col gap-6">
      @foreach ($steps as $index => $step)
        ...
        <div class="relative flex items-start min-h-[48px] group">
          ...
          @if (! $isLast)
            <div
              class="absolute left-5 top-10 bottom-0 w-0.5 {{ $status === 'completed' ? 'bg-success-500' : ($status === 'rejected' ? 'bg-danger-500' : 'bg-gray-200') }} transition-colors duration-300 z-0"
            ></div>
          @endif
          ...
          @if ($showLabels)
            <div class="ml-4 flex-1 pb-4 min-w-0">
              ...
              @if (isset($step['description']))
                <p
                  class="text-sm text-txt-black-600 mt-1 truncate"
                  title="{{ $step['description'] }}"
                >
                  {{ $step['description'] }}
                </p>
              ...
              <div
                class="flex items-center mt-2 space-x-4 text-xs text-txt-black-500"
              >
                @if ($showTimestamps && isset($step['timestamp']))
              ...
              @if (isset($step['comments']) && $step['comments'])
                <div
                  class="mt-2 p-3 bg-washed rounded-md border border-otl-divider"
                >
