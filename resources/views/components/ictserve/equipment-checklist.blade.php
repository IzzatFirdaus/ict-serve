...
<div
  class="bg-bg-white border border-otl-divider rounded-lg p-6"
  {{ $attributes }}
>
  {{-- Header --}}
  <div class="flex items-center justify-between mb-6">
    ...
    @if ($mode === "issue" || $mode === "return")
      <div class="text-sm text-txt-black-500">
        <span class="font-medium">Tarikh:</span>
  ...
  <div class="space-y-4">
    @foreach ($issuanceItems as $key => $label)
      ...
      <div
        class="flex items-start space-x-3 p-4 rounded-lg border {{ $isChecked ? "border-otl-success-200 bg-success-50" : "border-otl-gray-200 bg-gray-50" }} transition-colors"
      >
        {{-- Checkbox --}}
        <div class="flex items-center h-6">
          @if ($readonly)
            <div
              class="h-5 w-5 rounded flex items-center justify-center {{ $isChecked ? "bg-success-500" : "bg-gray-300" }}"
              aria-hidden="true"
            >
              @if ($isChecked)
                {{-- MYDS 20x20 icon --}}
        ...
        <div class="flex-1 min-w-0">
          <label
            for="{{ $itemId }}"
            class="block text-sm font-medium text-txt-black-900 cursor-pointer"
          >
            {{ $label }}
          </label>
          <span id="{{ $itemId }}-desc" class="sr-only">
            Tandakan jika item diterima
          </span>
...
{{ $notes }}</textarea
        ...
        <div class="flex-shrink-0">
          @if ($isChecked)
            <div
              class="h-6 w-6 rounded-full bg-success-100 flex items-center justify-center"
              aria-hidden="true"
            >
              <svg
                class="h-4 w-4 text-txt-success"
                viewBox="0 0 20 20"
                fill="none"
                stroke="currentColor"
                stroke-width="1.5"
  ...
  @if ($readonly)
    @php
      ...
      $checkedItems = collect($checklist)
          ->where("checked", true)
          ->count();
    ...
    >
      <div class="flex items-center justify-between mb-2">
        <span class="text-sm font-medium text-txt-black-700">
          Status Kelengkapan
        </span>
        <span class="text-sm font-bold text-txt-black-900">
