    ...
    <section class="px-6 py-6 grid gap-6">
        ...
        <div>
          ...
          >
            ...
            @foreach ($categories as $category)
              <option
                value="{{ $category['id'] }}"
                {{ ($ticket['category'] ?? '') === $category['id'] ? 'selected' : '' }}
              >
                {{ $category['name'] }}
              </option>
            @endforeach
      ...
      <div>
        ...
        >
          ...
          <option
            value="no_impact"
            {{ ($ticket['impact_assessment'] ?? '') === 'no_impact' ? 'selected' : '' }}
          >
            Tiada Kesan - Kerja dapat diteruskan seperti biasa
          </option>
          <option
            value="minor_impact"
            {{ ($ticket['impact_assessment'] ?? '') === 'minor_impact' ? 'selected' : '' }}
          >
            Kesan Kecil - Sedikit gangguan tetapi kerja dapat diteruskan
          </option>
          <option
            value="moderate_impact"
            {{ ($ticket['impact_assessment'] ?? '') === 'moderate_impact' ? 'selected' : '' }}
          >
            Kesan Sederhana - Kerja terganggu, perlu penyelesaian segera
          </option>
          <option
            value="major_impact"
            {{ ($ticket['impact_assessment'] ?? '') === 'major_impact' ? 'selected' : '' }}
          >
            Kesan Besar - Kerja terhenti, perlu penyelesaian segera
          </option>
          <option
            value="critical_impact"
            {{ ($ticket['impact_assessment'] ?? '') === 'critical_impact' ? 'selected' : '' }}
          >
            Kesan Kritikal - Kerja terhenti sepenuhnya, penyelesaian kecemasan
            diperlukan
          </option>
        </x-myds.select>
      ...
      <section class="px-6 py-6 grid gap-6">
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            >
              Format disokong: JPG, PNG, PDF, DOC, DOCX, TXT. Maksimum 5 fail,
              10MB setiap fail.
            </p>
            @error('attachments.*')
              <x-myds.error>{{ $message }}</x-myds.error>
          ...
          <div
            class="bg-info-50 border border-info-200 rounded-lg p-4 flex items-start gap-3"
          >
            <span
              class="inline-flex items-center justify-center rounded-full bg-info-100 text-info-600 w-6 h-6"
              aria-hidden="true"
            >
            ...
            <span class="text-sm text-info-700">
              <strong>Petua:</strong>
              Sertakan tangkapan skrin mesej ralat atau foto kerosakan untuk
