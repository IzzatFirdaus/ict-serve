...
<div class="myds-field space-y-1" @if($maxlength) x-data="{ count: 0, max: {{ $maxlength }} }" @endif>
    @if($label)
    ...
    @if($maxlength)
        <div id="{{ $textareaId }}-counter" class="flex justify-end text-xs text-gray-600 font-inter">
            <span x-text="count">0</span>/<span>{{ $maxlength }}</span> aksara
        </div>
    @endif
