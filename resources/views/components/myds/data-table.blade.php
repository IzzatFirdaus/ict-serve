...
<div class="bg-white dark:bg-dialog border border-divider radius-l shadow-card overflow-hidden" {{ $attributes }}>
    ...
    @if(count($data) > 0)
                    ...
                    <tr class="border-b border-divider">
                        @foreach($headers as $header)
                            <th scope="col" class="px-6 py-3 text-left">
                                ...
                                @else
                                    <span class="font-inter text-xs font-medium txt-black-700 dark:txt-black-300 uppercase tracking-wider">
                                        {{ $header['label'] }}
                                    </span>
                                @endif
                            </th>
                        @endforeach
                        @if(count($actions) > 0)
                            <th scope="col" class="px-6 py-3 font-inter text-xs font-medium txt-black-700 dark:txt-black-300 uppercase tracking-wider">
                                Tindakan
                            </th>
                        @endif
                    </tr>
        ...
        @if($pagination && $paginator)
            <div class="px-6 py-4 bg-gray-50 dark:bg-black-100 border-t border-divider">
                {{ $paginator->links() }}
            </div>
        @endif
    @else
        ...
        <div class="text-center py-12">
            @switch($emptyStateIcon)
                @case('table')
                    <x-myds.icons.table class="w-12 h-12 txt-black-400 mx-auto mb-4" aria-hidden="true"/>
                    @break
                @case('document')
                    <x-myds.icons.document class="w-12 h-12 txt-black-400 mx-auto mb-4" aria-hidden="true"/>
                    @break
                @default
                    <x-myds.icons.search class="w-12 h-12 txt-black-400 mx-auto mb-4" aria-hidden="true"/>
            @endswitch

            <h3 class="font-poppins text-lg font-medium txt-black-900 dark:txt-white mb-2">
                {{ $emptyStateTitle }}
            </h3>
