...
<fieldset
  class="grid gap-3"
  @if($legendId) aria-labelledby="{{ $legendId }}" @endif
  @if($hintId) aria-describedby="{{ $hintId }}" @endif
>
  @if ($label)
    <legend
      id="{{ $legendId }}"
      class="myds-label font-poppins txt-black-900 mb-1"
    >
      {{ $label }}
      @if ($required)
  ...
  @if ($hint)
    <div id="{{ $hintId }}" class="myds-hint text-sm txt-black-500 mb-1">
      {{ $hint }}
    </div>
  @endif
  ...
  @foreach ($options as $opt)
    ...
    <div class="flex items-start gap-3">
      ...
      <div>
        <label
          for="{{ $id }}"
          class="txt-black-900 font-medium cursor-pointer"
        >
          {{ $opt['label'] }}
        </label>
        @if ($hintOpt)
          <div id="{{ $hintOptId }}" class="myds-hint text-sm txt-black-500">
            {{ $hintOpt }}
          </div>
        @endif
      </div>
...
<div
  role="radiogroup"
  aria-labelledby="{{ $label ? $name . '-legend' : null }}"
  class="grid gap-3"
>
  @foreach ($options as $opt)
    @php
    ...
    <div class="myds-radio-container">
      ...
      <div>
        <label
          for="{{ $id }}"
          class="txt-black-900 font-medium"
