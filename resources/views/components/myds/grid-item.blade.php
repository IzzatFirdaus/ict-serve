<x-ictserve.grid-item {{ $attributes->merge([]) }}>
    {{ $slot }}
</x-ictserve.grid-item>
{{--
  Grid Item for ICTServe (iServe)
  - Standards: grid, spacing, tokens.
  - Principles: hierarchical, flexible, minimal.
  - Usage:
  <x-ictserve.grid-item span="6" md="4" sm="4" class="bg-white">...</x-ictserve.grid-item>
  - Props:
...
@php
  ...
  if ($sm) {
    $classes[] = 'sm:col-span-' . $sm;
  }
  if ($md) {
    $classes[] = 'md:col-span-' . $md;
  }
  if ($lg) {
    $classes[] = 'lg:col-span-' . $lg;
  }
@endphp

<{{ $as }}
  {{ $attributes->merge(['class' => implode(' ', $classes) . ' ' . $class]) }}
>
  {{ $slot }}
</{{ $as }}>
