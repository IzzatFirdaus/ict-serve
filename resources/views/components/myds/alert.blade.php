@props([
    'variant' => 'info',
    'title' => null,
    'dismissible' => false,
    'icon' => true,
])

@php
    $variantClasses = match($variant) {
        'success' => 'bg-success-50 border-otl-success-200 text-txt-success',
        'warning' => 'bg-warning-50 border-otl-warning-200 text-txt-warning',
        'danger' => 'bg-danger-50 border-otl-danger-200 text-txt-danger',
        'info' => 'bg-primary-50 border-otl-primary-200 text-txt-primary',
        default => 'bg-primary-50 border-otl-primary-200 text-txt-primary'
    };

    $iconSvg = match($variant) {
        'success' => '<path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path>',
        'warning' => '<path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"></path>',
        'danger' => '<path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"></path>',
        'info' => '<path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"></path>',
        default => '<path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"></path>'
    };

    $alertId = 'alert-' . uniqid();
@endphp

<div
    id="{{ $alertId }}"
    class="p-4 border rounded-[var(--radius-m)] {{ $variantClasses }}"
    role="alert"
    {{ $attributes }}
>
    <div class="flex">
        @if($icon)
            <div class="flex-shrink-0">
                <svg class="w-5 h-5 mt-0.5" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                    {!! $iconSvg !!}
                </svg>
            </div>
        @endif

        <div class="{{ $icon ? 'ml-3' : '' }} flex-1">
            @if($title)
                <h3 class="text-sm font-semibold mb-1">
                    {{ $title }}
                </h3>
            @endif

            <div class="text-sm {{ $title ? '' : 'font-medium' }}">
                {{ $slot }}
            </div>
        </div>

        @if($dismissible)
            <div class="ml-auto pl-3">
                <button
                    type="button"
                    class="inline-flex rounded-[var(--radius-s)] p-1.5 hover:bg-black hover:bg-opacity-10 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-fr-primary transition-colors"
                    onclick="document.getElementById('{{ $alertId }}').remove()"
                    aria-label="Dismiss alert"
                >
                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                        <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path>
                    </svg>
                </button>
            </div>
        @endif
    </div>
</div>
