...
- Added `resources/views/public/motac-info.blade.php` (styled public page, Bahasa Melayu)
...
- Ran frontend `npm ci`, `npm run lint:style` and `npm run build` (lint reported design token issues to triage)
...
- [ ] Confirm page content is displayed in Bahasa Melayu and matches project typography.
...
- [ ] Frontend: `npm ci` then `npm run lint:style` and `npm run build` — address lint findings as needed.

# Notes & Known Issues

- `npm run lint:style` reported 63 files using Tailwind utilities, inline styles, or hex colors. These must be triaged separately to fully comply with design tokens.
...
- @frontend-maintainer (style lint fixes)
